...
export default function ImportDialog({ bookId, onClose }) {
  ...
  const handleFile = useCallback(async (f) => {
    ...
    setUploading(true)
    try {
      const data: any = await uploadDocument(bookId, f)
      setDocId(data.id)
      const detectData: any = await detectChapters(bookId, data.id)
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      const data: any = await importChapters(bookId, docId, body)
