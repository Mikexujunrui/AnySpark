...
function looksLikeEdit(instruction: string) {
...
function Field({ label, icon, children, hint = undefined }: { label: string; icon?: string; children: React.ReactNode; hint?: string }) {
  return (
    <div>
...
export default function AutopilotModal({ bookId, onClose, onTaskCreated, onOpenTransform }) {
  ...
  const handleDirectStart = async () => {
    ...
    setError('')
    try {
      const result: any = await api.startAutopilot(bookId, buildPayload(false))
