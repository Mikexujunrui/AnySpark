...
export default function CharacterHeatmap({ bookId }) {
  ...
  async function load() {
    setLoading(true)
    try {
      const res = await api.getCharacterMentions(bookId)
      setData(res)
      if (!res.matrix) {
  ...
  function render() {
    ...
    if (matrix.length === 0 || chaptersCount === 0) {
      svg.attr('width', w).attr('height', 120)
    ...
    const colorScale = d3.scaleSequential()
      .domain([0, maxCount])
      .interpolator(t => d3.interpolate('#1e293b', '#0ea5e9')(Math.pow(t, 0.55)))

    // Horizontal scroll container (x-axis may exceed viewport)
    const scrollWrap = d3.select(containerRef.current).selectAll('.heatmap-scroll').data([0]).join('div')
      .attr('class', 'heatmap-scroll')
      .style('overflow-x', 'auto')
      .style('max-width', `${w}px`)
      .style('position', 'relative')
    scrollWrap.selectAll('*').remove()
    ...
    // Remove original svg contents (re-render in scrollWrap instead)
    svg.selectAll('*').remove()

    // Y axis (character labels)
    sg.selectAll('.row-label').data(matrix).enter().append('text')
    ...
    // X axis (chapter indices) — drawn above rows
    ...
    // Cells
    const tooltip = d3.select(containerRef.current).selectAll('.hm-tip').data([0]).join('div')
      .attr('class', 'hm-tip')
    ...
    matrix.forEach((row, ri) => {
      ...
      for (let ci = 0; ci < chaptersCount; ci++) {
        ...
        if (count > 0) {
          rect.attr('stroke', '#0ea5e9').attr('stroke-width', 1.5).attr('stroke-opacity', 0.0)
    ...
    // Color legend
    const legendW = 160
    const legendG = sg.append('g').attr('transform', `translate(${innerW + 15}, ${innerH / 2 - legendW / 2})`)
      .attr('class', 'legend-x')
    if (innerW + legendW + 20 < w) {
      legendG.attr('transform', `translate(${innerW - legendW - 10}, ${innerH + 20})`)
        .attr('class', null)
    } else {
      legendG.remove()
    }
    // inline legend under title instead (simpler)
    sg.selectAll('.legend-x').remove()
  }
        ...
        <div className="flex gap-4 text-xs text-zinc-500">
          ...
          {topChar && (
            <span className="flex items-center gap-1.5">
              <Icon name="award" size={12} className="text-amber-400" /> 主角 <b className="text-zinc-300">{topChar.charName}</b>
        ...
        <div className="flex gap-0.5">
          {[0.05, 0.2, 0.4, 0.65, 0.9].map((t, i) => (
            <div key={i} className="w-5 h-3 rounded-sm"
              style={{
                background: d3.interpolate('#1e293b', '#0ea5e9')(t)
              }} />
          ))}
        </div>
