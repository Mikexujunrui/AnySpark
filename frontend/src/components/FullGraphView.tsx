          ...
          {selectedNode && !showRelModal && (
            <div className="p-3 border-b border-zinc-800">
              ...
              {selectedNode.data && Object.entries(selectedNode.data).slice(0, 4).map(([k, v]: [string, any]) => {
                const displayText = typeof v === 'string' ? v : (typeof v === 'object' && v !== null ? JSON.stringify(v) : String(v))
                return (
                  <p key={k} className="text-[10px] text-zinc-500 truncate">{k}: <span className="text-zinc-300">{displayText.slice(0,30)}</span></p>
                )
              })}
            </div>
          )}
