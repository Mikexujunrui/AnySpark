...
import PlotCardSelector from './PlotCardSelector.jsx'
import QuestionCard from './QuestionCard.jsx'
import WorkflowProgress from './WorkflowProgress.jsx'
import PatchNotification from './PatchNotification.jsx'
import Icon from '../ui/Icon.jsx'

const MemoizedMarkdown = memo(function MarkdownContent({ text }) {
  return (
...
export default function MessageList({
  ...
  return (
    ...
    >
          ...
          <div className={`flex flex-col max-w-[min(640px,90%)] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
            ...
            <div className={`rounded-xl px-4 py-3 text-sm leading-relaxed ${
              ...
              {editingIdx === i ? (
                ...
                <>
                  {msg.parts && <TurnParts parts={msg.parts} />}
      ...
      {(streaming || uploading) && (
  (messages[messages.length - 1]?.role === 'agent' && !messages[messages.length - 1]?.text) ||
