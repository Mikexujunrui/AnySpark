...
export default function AutopilotConsole({ state, taskId, bookId, sessionId: _sessionId, onPause, onResume, onCancel, onForceStop, onSkip, onClose }) {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden" style={{ color: '#e0e0e0' }}>
      ...
      <div className="shrink-0 p-3 border-b border-zinc-800" style={{ background: '#1a1a2e' }}>
        ...
        {state?.disconnected && (
          <div className="mt-2 text-xs text-red-400 flex items-center gap-1">
            <Icon name="alert-triangle" size={12} /> 后端连接已断开，任务可能已失联
          </div>
        )}
      </div>
      ...
      <div className="shrink-0 p-3 border-t border-zinc-800 flex gap-2 flex-wrap" style={{ background: '#1a1a2e' }}>
        ...
        {/* Force stop: always visible, no confirm, works even when disconnected */}
        {state?.status !== 'completed' && state?.status !== 'cancelled' && onForceStop && (
          <button onClick={onForceStop} style={{ ...btnStyle('#8b0000'), fontWeight: 'bold' }}>
            <Icon name="x" size={11} className="mr-1" strokeWidth={3} />强制中止
          </button>
        )}
      </div>
    </div>
