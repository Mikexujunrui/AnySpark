...
export default function RunLedger({ metrics }) {
  ...
  const hallucinationCount = metrics.hallucination_hits
    ? Object.values(metrics.hallucination_hits).reduce((a, b) => a + b, 0)
    : 0
  const hasIssues = hallucinationCount > 0 || (metrics.drift_corrections ?? 0) > 0
