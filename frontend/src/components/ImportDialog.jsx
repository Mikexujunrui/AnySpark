...
import { uploadDocument, detectChapters, importChapters, batchExtractKnowledge } from '../api.js'

const STEPS = ['upload', 'review', 'import']

export default function ImportDialog({ bookId, onClose }) {
  ...
  const handleFile = useCallback(async (f) => {
    ...
    setUploading(true)
    try {
      const data = await uploadDocument(bookId, f)
      setDocId(data.id)
      const detectData = await detectChapters(bookId, data.id)
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      const data = await importChapters(bookId, docId, body)
    ...
    setImporting(false)
  }, [bookId, docId, detectResult, extractKnowledge, volumeName])
