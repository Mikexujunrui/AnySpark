      ...
      if (msg.startsWith('/w ') || msg.startsWith('/ws ')) {
        ...
        for await (const event of parseSSE(res)) {
          ...
          if (event.type === 'progress') {
            ...
            onProgress?.(null)
            const msgText = data.totalEntities || data.totalRelations
              ? `${(data.message as string) || ''}\n实体: ${data.totalEntities || 0}  |  关系: ${data.totalRelations || 0}  |  伏笔: ${data.totalForeshadows || 0}`
              : (data.message as string) || ''
            // Always show the done message unless it's a trivial placeholder
            // that was already covered by streaming chunks. Streaming chunks
            // may be empty or partial, so the done message is the authoritative
            // final report.
            const isTrivial = !msgText || msgText === '完成' || msgText === '操作已取消'
            if (!isTrivial || !streamingStarted) {
              if (msgText && mountedRef.current) onMessage?.({ type: 'plain', text: msgText })
            }
            onKnowledgeChanged?.()
