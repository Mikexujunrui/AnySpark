...
> 基于 3.2.7 开发，暂不发布、不改变书籍数据格式，待多轮模拟与真实使用测试后再决定提交方式。

### Agent 稳定性

- 正文写作能力包同时暴露 `patch_chapter` / `edit_chapter`，AI 草稿被新章覆盖保护拦截后仍可在同一轮进行版本化修改
- 自主模式按“书籍＋会话”隔离；`patch_chapter`、`edit_chapter` 等可从历史回退的编辑不再反复确认，删除章节、版本、实体及清空历史仍必须确认
- `patch_chapter` 的空数组在权限弹窗前即判为参数错误；同轮相同修改参数禁止重复调用，同章最多尝试四次版本化修改，避免确认死循环
- 新增正文拒答守卫：模型以正常响应返回安全过滤、内容政策或拒绝说明时，本次结果明确标记“未保存”，不创建受保护占位章节

### 文学工作流

- 新增默认关闭的“首章200字钩子编辑”和 `/opening_200_audit` Skill，只审查开篇进入速度、具体性、信息负担、有效阅读问题和模板化痕迹，不自动改稿
- `run_review` 支持 `excerpt_limit`，短片段评审不再把整章偷偷送入模型，减少 token 浪费
- 章节交接卡改为证据化的开头/结尾状态：时间、地点、伤势、持有物和未完成动作只有在相邻章节两侧均为高置信且有原文短句时才报告冲突，缺失信息不会被误判为矛盾
- 写作准备、流式写作与普通写作统一读取结构化交接卡；长篇续写不再只依赖一段自由文本摘要
- 时间线增加主时间、回忆、预叙与同期支线，保存绝对时间、相对时间、来源证据和置信度；时间线页同时展示每章主时间范围及交接冲突数
- 新增“剧情规范库”：可保存节奏、冲突、视角、必须项和禁用套路，只有用户显式启用的规范才以紧凑格式进入正文提示词

### 参考书与任务恢复

- 新增参考书后台分析任务：按固定章节数建立输入指纹检查点，结构、文风、句式、修辞、谶语、视角与情感弧线逐维度缓存
- 分析任务立即返回并显示真实进度；单项失败可从已完成检查点重试，应用中途退出后任务会标记为可恢复，不再让页面等待到 HTTP 读超时

### 作者 DNA 实验室（实验性功能，默认关闭）

- 设置新增独立的“实验性功能”页面；作者 DNA 总开关升级后默认关闭，由用户自行决定是否承担额外 API 调用与试验风险
- 新建和编辑项目可选择“从零原创”或“原作续写”；只有“全局实验开关已开启＋当前项目为原作续写”时才显示作者 DNA 入口并允许正文注入
- 前端隐藏不是唯一保护：全部作者 DNA API、场景合同和正文上下文注入均执行相同双重门控；关闭功能或改回原创只停用、不删除既有分析
- 后台分析期间关闭实验开关时，当前请求完成后会在下一检查点中止并保留进度，避免继续产生后续 API 消耗
- 参考书按作品、章节和文本块生成稳定证据 ID，展示每部作品前中后四段覆盖；分析前明确显示预计模型调用次数
- 六层语义蒸馏覆盖故事发动机、人物行为、场景语法、叙事镜头、文字风格和节奏展开；逐批证据账本、逐层综合与交叉审计均可从检查点恢复
- 模型生成的规则默认处于“待复核”，只有用户逐层确认后才进入正文上下文；移除来源参考书后，相应缓存规则自动停止注入
- 用户原作理解独立于原作事实和作者 DNA 保存，可用候选原文块核验为“强支持、合理、歧义、弱支持或冲突”，再由用户决定是否晋升为本次续写的解释 Canon
- 新增当前场景 Writer Package：分别保存创作意图、剧情功能、人物状态、相关 Canon、新 Canon、Beats、结束状态和 Stop Anchor；API 丢弃任何未来剧情字段
- 正文模型不再看到后续事件名称；未来剧情只提供给独立边界校验器。场景合同启用时，以当前场景检索样本替代随机通用样本，避免重复上下文和 token 浪费
- `.spark` 迁移会保留项目用途、已确认规则、用户解读和当前场景合同，但不复制参考书原文；实验总开关属于本机偏好不会随归档开启，迁移结果标记为“证据已分离”

### 导出与迁移

- 桌面窗口导出 TXT、DOCX、EPUB、`.spark` 时调用系统保存面板，完成后持续显示并可复制实际保存路径；浏览器模式明确提示路径由浏览器下载设置决定
- `.spark` 归档新增交接卡和剧情规范，并完整往返带证据的故事时间字段
- 修复从另一台电脑导入 `.spark` 时，新书籍 ID 与归档旧 ID 不同导致知识图谱、关系、伏笔及时间线被静默过滤的问题

### macOS 构建

- PyInstaller 的原始 `.app` 全部改在 `/tmp` 构建，`dist` 只保留 DMG/ZIP；构建成功或失败都会清理临时目录
- DMG 创建后执行完整性验证但不自动挂载，并加入挂载残留保护；`build`/`dist` 禁止残留可被 Spotlight 误认作安装项的 `.app`

### 验证

- `pytest`：907 passed，1 skipped；`ruff` 与 mypy 增量门禁均为 0 新错误
- TypeScript `tsc --noEmit`、Vite 生产构建和 ESLint 通过（0 error；存量 warning 不阻塞）
- macOS arm64 DMG 通过 `hdiutil verify`，ZIP 内应用通过深度签名校验
- 使用隔离数据目录完成独立窗口冒烟：创建书籍、保存并启用剧情规范、系统保存面板导出 TXT、路径回显、关闭窗口后服务与端口均正常退出
- 构建前后无残留 `.app`、无 AnySpark 挂载卷；`dist/.metadata_never_index` 持续阻止测试安装包污染 Spotlight 搜索



## 3.2.7 - 2026-08-04

- 版本发布（由 scripts/release.py 生成，详细信息见 git log）。

## [3.2.6] - 2026-08-02

> 模型参数精细化 + 稳定性修复版。不改变项目数据格式，可覆盖安装。

### 新增

- **思考强度按模型族定制**：统一五档标尺（off/minimal/low/medium/high），
  各模型族声明真实可用档位——DeepSeek 3 档、OpenAI o 系列 4 档、
  Anthropic/Gemini 思考预算式；未知档位就近映射，`settings.json` 可扩展自定义模型族档位
- **extract_all_chapters 批量提取修复**：修复该工具未注册导致不可用的问题，
  现支持对全书一次性批量提取知识（内容指纹缓存跳过未变化章节、单章超时防护、可中途取消）

### 修复

- **章节序号零号错位**：章节侧边栏普通章节序号从 0 开始显示（`#0` 表示第一章），
  与系统 1-based 约定冲突导致 Agent 混淆；改为 `#N` 并修复卷内跳号
- **delegate_writing 章节号误解析**：指令含连续性上下文（"承接第6章结尾，写第7章"）
  时误判目标章节触发"已存在"保护，Agent 反复重试形成死循环；改为目标型优先 + 末位回退

## [3.2.5] - 2026-08-01

> 稳定性修复版：合并上游 3.2.1 稳定性改动中与自由创作理念一致的 A 类修复。
> 不改变项目数据格式，可覆盖安装。

### Agent 稳定性

- **消息队列真正消费**：运行中追加的消息会在每轮开始及终止前注入当前 Agent，
  不再长期停留在“消息已排队”状态；会话活跃时间在开始、入队和取队时正确更新
- **headless 假完成拦截**：`llm_error`、空响应、Token/轮次上限等非成功终态不再恒报成功；
  Auto 不会再把未写成的章节当作成品走质量门
- **失败步骤显式传播**：步骤未返回成功结果时抛错进入重规划，不再静默继续
- **持久化锁修复**：headless 记录改用按书加锁 `book_lock(book_id)`

### 规划与指令理解

- **中文章号解析**：支持“第三章”“第三到八章”等中文数字章号（不含 LLM 往返）
- **“写第N章后检查”意图修正**：带章号目标的写作指令不再被错分为纯分析任务

### 前端体验

- **中文输入法**：IME 组字期间的回车只确认候选词，不再提前发送消息
- **长文本自动换行**：普通段落、Markdown、无空格长串与用户消息均限制在气泡宽度内；代码块保留横向滚动
- **写作预览可交互**：可收起、关闭并拖动分隔线调整右侧宽度；写作失败或用户中止时显示“未保存”和具体原因，不再把失败伪装成已保存
- **写作结束事件**：前端始终收到带 `saved`/`error`/`partial` 状态的结束事件

### 其他

- 评审工具、创作宪法开关等均维持现状，未引入新增创作护栏
...
- **创作宪法改为可选**：不再强制"先制定宪法才能进入对话"。宪法为空时可直接进入（仅显示一条非阻塞建议条），支持"插入推荐模板"、"启用并编辑"与"关闭"（`constitutionEnabled` 前后端开关补齐）；模板从自动预填改为按需插入，避免把"续写型连载"假设强加给重写/同人场景。
...
> 多平台打包、工具协议修复、Token 缓存和升级数据保护，并加入本节全部文学创作改进。

### 新增

- **独立桌面窗口（3.2.0 同版本更新）**：macOS 与 Windows 打包版改用系统 WebView
  显示完整应用窗口，不再自动打开默认浏览器；macOS 正常显示 Dock 图标，Windows 正常显示
  任务栏窗口
- **桌面进程生命周期**：可见窗口与本地 FastAPI 服务由同一个桌面外壳管理；关闭窗口或
  `⌘Q`/退出应用时会停止后台服务，覆盖安装时不再需要寻找隐藏的浏览器后端进程
- **单实例唤醒**：重复双击 AnySpark 不会启动第二个服务或占用端口，而是恢复并聚焦已经
  打开的窗口；检测到仍在运行的旧菜单栏版时会显示可见提示
- **创作宪法**：每本书首次进入对话前必须保存项目级创作规则；规则同时注入普通 Agent、
  直接写作、逐节点写作、剧情链复写和 Auto 写作，不依赖单轮聊天记忆
- **正文生成参数**：设置页新增 `temperature`、`top_p`、频率惩罚、存在惩罚和最大输出
  Token；只作用于正文/编辑模型，不影响工具调用 Agent 的结构稳定性
- **可执行文学 Skills**：`/literary_continuation`、`/author_style_continuation`、
  `/canon_audit`、`/surgical_literary_revision`，以及修复后的导入、精修、灵感、日常写作 Skill
- **Skill 真实路由**：`/技能名` 会把 YAML 中的指令、固定步骤、护栏与输出要求转换为实际
  Agent 工作流；自定义 Skill 也可通过同一入口执行
- **参考书用途**：每本参考书可设为“只学文风”“只作原著设定”“文风＋设定”；新加入的
  参考书默认“只学文风”
- **作者文风证据**：写作上下文可注入结构统计、句长/段落/对话密度、句式韵律、修辞、
  谶语、叙事视角和分散抽取的小样本，并明确禁止复用句子及无关作品事实
- **章节来源与保护标记**：导入原稿、参考书原稿、AI 草稿、AI 中断草稿具有明确来源，
  旧项目迁移后默认按受保护原稿处理
- **手动 Release 构建**：GitHub Actions 除标签发布外支持网页点击运行；Windows EXE/安装器
  与 macOS DMG/ZIP 会保留为可下载的 Actions Artifacts
...
- **章节号误判**：不再把“写 2500 字”中的 `2500` 当成第 2500 章；正确识别“第十一章”
  “第二十章”等中文章节号
- **正文生成后又消失**：同一个模型响应和同一 Agent 运行只允许一个全章生成工具；
  后续 `write/edit/store/rewrite` 调用会被互斥保护取消
- **逐节点暗中替换**：发现禁止角色时保留当前草稿并交给评审，不再在后台重写节点、
  删除刚刚流式显示的正文
- **流式中断丢稿**：连接、限流或服务中断后，只要已有足够正文就保存为“部分草稿”，
  明确要求从断点续写而不是整章重来
- **Auto 质量门顺序**：先评审草稿，通过后才把新事实提取进知识库；质量不通过或评审异常时，
  所有审计模式都暂停并保留草稿，不再隐式整章重写或带错继续挂机
- **Auto 无新章崩溃/误写**：大纲章节全部存在时返回安全的空计划，提示先新增后续大纲，
  不会把已有章节重新当作待写任务
- **自主模式越权**：安全自主模式只连续执行读取、分析、评审和新建草稿；删除、局部改原稿、
  整章覆盖、批量变换和历史清理仍必须显式确认
- **参考书事实串书**：“只学文风”参考书从底层禁止 `search_reference` 事实搜索和
  `migrate_reference_knowledge` 迁移，不再只依靠提示词约束
- **第三方参数兼容**：OpenAI 兼容中转不支持惩罚参数时，自动降级到通用参数重试，
  不因设置页新参数导致整次正文生成失败
- **旧 Skill 无效工具**：删除 `compare_versions`、`store_inspiration` 等不存在的步骤，
  避免 Skill 看似可用、执行时却找不到工具
- **CLI 版本漂移**：`--version` 统一读取项目版本，不再固定显示过时的 `0.5.0`

### 安装、覆盖升级与数据安全

- 本次独立窗口改造仍属于 `3.2.0`，没有改变数据格式；可直接覆盖此前的 3.2.0 安装包。
  第一次覆盖前应从旧版菜单栏 `✦ → 退出火花` 完全退出，之后的更新可直接从 Dock/任务栏退出
- macOS 可把新 `AnySpark.app` 直接拖入 `/Applications` 并选择替换；Windows 使用同一
  AppId 安装到原目录即可覆盖升级
- 书籍、API 设置、创作宪法、Skills 与历史版本位于系统用户数据目录，不在应用安装目录内：
  macOS 为 `~/Library/Application Support/AnySpark`，Windows 为 `%APPDATA%\AnySpark`
- 每次检测到打包版版本变化，启动前自动把现有 `data/` 生成
  `backups/pre-upgrade_旧版_to_新版_时间.zip`
- Windows 卸载/覆盖安装不会删除 `%APPDATA%\AnySpark`；旧便携版数据首次迁移采用复制，
  原目录保留为恢复副本
- `.spark` 项目归档会携带书名、描述和创作宪法，可用于另一台电脑导入
...
- **跨平台安装包**：Release 工作流同时在真实 Windows 与 macOS Runner 构建，发布 Windows x64 安装版/便携 ZIP，以及 macOS arm64 DMG/ZIP
- **Windows 安装器**：新增 Inno Setup 安装脚本，使用稳定 AppId 支持同目录覆盖升级，可选桌面快捷方式
- **模型列表自动发现**：API 设置支持从 OpenAI 兼容、Anthropic 与 Gemini 端点拉取模型，支持搜索、多选与刷新，不再要求手写完整模型名
- **项目归档导入**：书架可直接导入另一台电脑导出的 `.spark` 项目归档
- **知识证据锚点**：AI 提取的新实体、更新与关系必须能在章节原文中找到名称证据；实体卡保存章节、命中名称及原文摘录
- **提取内容指纹缓存**：全书或单章重复提取时，内容未变化的章节自动复用成功结果，显示 `0 token`；可用 `force=true` 强制重建

### 修复

- **并行工具消息 400**：修复多个 `extract_chapter` 等工具并行返回时，被中途插入 user 提示而破坏 `assistant(tool_calls) → tool...` 连续协议的问题
- **工具历史状态机**：消息清理从“全局检查 ID 是否存在”升级为逐 assistant 批次重排，自动修复乱序、缺失、重复/空 ID，并丢弃孤儿结果
- **缺少 API Key 日志风暴**：Agent 启动前检查当前模式所需的供应商、API Key 与模型；配置不完整时在界面显示可操作错误，不再递归创建客户端
- **定稿顺序错误**：`finalize_chapter` 改为先验证、再做有证据的知识提取和连续性处理，最后标记 `final`
- **hard 约束未生效**：用户定义的 hard 叙事约束被违反时，章节保持/回退为 `draft`，阻止知识入库与连续性卡片更新
- **设置写入中断风险**：`settings.json` 改为临时文件写入后原子替换

### Token 与质量优化

- 删除知识提取后额外注入的“请再次汇报”模型轮次；工具结果已有完整摘要，最终回复过短时直接复用工具摘要
- 普通聊天不再无条件调用意图分类模型；只有包含大纲、世界观、时间线或地点地图关键词时才分类
- 无原文依据的 AI 知识提案不会进入正式知识库，过滤数量会显示在提取报告中
- 生成章节仍保存为可编辑草稿；只有显式定稿流程才可进入正式状态，避免警告报告被误认为质量闸门

### 安装与数据安全

- macOS 用户数据固定保存在 `~/Library/Application Support/AnySpark`
- Windows 用户数据从 EXE 旁迁移至 `%APPDATA%\AnySpark`，安装目录只保存程序文件
- Windows 旧便携版使用覆盖解压/同目录安装时，首次启动自动复制旧 `data`、`.env` 与 `config.json`；源目录不会删除
- 打包版检测到应用版本变化时，在用户数据目录的 `backups/` 自动生成升级前 ZIP 备份
- DMG 覆盖 `/Applications/AnySpark.app`、Windows 相同 AppId 覆盖安装或卸载程序，均不会删除系统用户数据目录

### 验证

- `ruff check src/ tests/`：0 errors
- `pytest`：670 passed，3 skipped
- TypeScript 类型检查与 Vite 生产构建通过
- macOS 3.1.0 打包成品以 `--headless` 启动，`/api/health` 与版本接口返回 200
- 新增工具协议、知识证据、内容指纹缓存、模型配置预检、旧数据迁移和升级备份回归测试
- Windows 产物由 `windows-latest` 构建；macOS 产物由 `macos-14` 构建
