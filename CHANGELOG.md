...
> 稳定性与文学证据更新。本版不改变项目数据格式，可直接覆盖 3.2.0。

### 文学评审与反幻觉

- **原著文风一致性专家获得真实证据**：`needs_knowledge` 改为启用，评审上下文注入当前书籍的
  知识库事实、非当前待评章节的正文样本、参考书原文样本与已缓存文风分析
- **参考书文风/事实边界**：证据显式标记 `style` / `canon` / `both`；只学文风的参考书不得被当作本书事实
- **评审团作品串线拦截**：评审员无证据提及《红楼梦》等外部作品时自动纠正一次；再次出现则拒绝该评审结果
- **评审员间防污染**：串行评审只传递结构化问题与建议，不把前一位评审员的推断冒充为事实；
  汇总阶段只能组织已有意见，不能新增作品名、人物或“原文引用”

### Agent 与 Auto 稳定性

- **消息队列真正消费**：运行中追加的消息会在每轮开始及终止前注入当前 Agent，不再长期停留在
  “消息已排队”状态；会话活跃时间在开始、入队和取队时正确更新
- **Auto 假完成拦截**：`llm_error`、空响应、Token/轮次上限和未完成任务均为失败终态；
  任务只有在目标章节已存在且正文非空时才能通过写作步骤与检查点
- **Auto 目标解析与原稿保护**：正确识别“第三章”等中文章号；“写第三章后检查”不再被错分为纯分析；
  写新章步骤命中已有章节时在执行前拒绝覆盖
- **LLM 规划不再可信任执行**：写作步骤必须有 `chapter_index`、至少 10 轮预算并明确调用
  `write_chapter`/`delegate_writing`；失败后重规划返回空数组不再把整个任务标成完成
- **后台对话持久化**：改用现有 `load_messages` + `save_messages` 接口，修复
  `JsonStore has no attribute append_message`，后台执行记录可正常追溯
- **角色阶段工具**：修复 `set_character_phase` 调用不存在的 `character_entity_id` 参数而中断知识库建设
- **超时重试**：普通 `The read operation timed out` 现在可识别为可重试连接错误；同步调用最多重试
  1 次、流式调用最多 2 次，避免 180 秒超时被多层重试放大成长时间假死

### 前端体验

- **中文输入法**：IME 组字期间的回车只确认候选词，不再提前发送消息
- **长文本自动换行**：普通段落、Markdown、无空格长串与用户消息均限制在气泡宽度内；代码块保留横向滚动
- **写作预览可交互**：可收起、关闭并拖动分隔线调整右侧宽度；写作失败或用户中止时显示
  “未保存”和具体原因，不再把失败伪装成已保存

### 安装、数据与验证

- Windows/macOS 继续使用固定安装识别与系统用户数据目录，覆盖安装不删除书籍、知识库、聊天或 API 设置
- 检测到 3.2.0 → 3.2.1 后仍会在启动前生成 `pre-upgrade_3.2.0_to_3.2.1_*.zip`
- `pytest`：710 passed，3 skipped；3.2.1 新增的评审证据与稳定性回归测试全部通过
- Vite 8.1.5 生产构建通过
...
- 新增灵感工具实现测试（4 个用例）。合并 3.2.1 稳定性回归后，测试套件为 **710 passed，3 skipped**。
