...
class TestAutopilotExecutor:
    ...
    async def test_get_status(self, tq, monkeypatch):
        ...
        steps = [
            TaskStep(id="s0", type="checkpoint", label="ch1完成",
                     config={"final": True, "chapter_index": 1}),
            TaskStep(id="s1", type="checkpoint", label="ch2完成",
                     config={"final": True, "chapter_index": 2}),
        ]
        task = PersistentTask(
            id="ap_test", type="autopilot", book_id="b1",
    ...
    def test_update_tokens(self, tq, monkeypatch):
        ...
        task = PersistentTask(
            id="ap_t", type="autopilot", book_id="b1",
            session_id="s1", metadata={"tokens_used": 0, "token_budget": 1000},
        )
        tq.create_task(task)
