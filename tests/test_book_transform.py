...
def book_with_chapters(tmp_data_dir):
    ...
    chapters = [
        ("第一章 初入江湖", "少年李逸站在山门前，望着云雾缭绕的青云山。小姐姐李婉从身后走来，轻声道：哥哥，我们真的要上山吗？" * 10),
...
async def test_find_replace_literal(book_with_chapters):
    ...
    result = await find_replace_book(loop, {
        "pattern": "小姐姐",
        "replacement": "妹妹",
        "scope": "all",
    }, book_with_chapters)
...
async def test_apply_directive_parallel(book_with_chapters):
    ...
    with patch("tools._common._call_edit_llm", side_effect=mock_edit_llm):
        loop = asyncio.get_event_loop()
        result = await apply_directive_globally(loop, {
...
async def test_transform_chapters_batch(book_with_chapters):
    ...
    with patch("tools._common._call_edit_llm", side_effect=mock_edit_llm):
        loop = asyncio.get_event_loop()
        result = await transform_chapters_batch(loop, {
            "chapter_ids": "#1-#2",
            "instruction": "增加环境描写",
            "mode": "patch",
        }, book_with_chapters)
...
async def test_restyle_book_missing_style(book_with_chapters):
    ...
    loop = asyncio.get_event_loop()
    result = await restyle_book(loop, {
...
def test_executor_dispatch():
    """Verify executor has dispatch branches for new tools."""
    import inspect

    from tools.executor import execute_tool_streaming

    source = inspect.getsource(execute_tool_streaming)
    tools = [
        "apply_directive_globally",
        "find_replace_book",
        "transform_chapters_batch",
    ...
    for t in tools:
        assert t in source, f"Tool {t} should have a dispatch branch in execute_tool_streaming"
