...
def test_add_and_list_task(scheduler):
    task = ScheduledTask(
        id="sched_test", name="测试任务", template="review",
        book_id="testbook", schedule_type="manual",
...
def test_update_task(scheduler):
    task = ScheduledTask(
        id="sched_test2", name="原始名", template="review",
        book_id="testbook", schedule_type="manual",
        created_at=datetime.now().isoformat(), updated_at=datetime.now().isoformat(),
    )
    scheduler.add_task(task)
...
def test_delete_task(scheduler):
    task = ScheduledTask(
        id="sched_test3", name="待删除", template="review",
        book_id="testbook", schedule_type="manual",
        created_at=datetime.now().isoformat(), updated_at=datetime.now().isoformat(),
    )
    scheduler.add_task(task)
...
def test_manual_task_no_next_run(scheduler):
    task = ScheduledTask(
        id="sched_test4", name="手动任务", template="review",
        book_id="testbook", schedule_type="manual",
        created_at=datetime.now().isoformat(), updated_at=datetime.now().isoformat(),
    )
    scheduler.add_task(task)
...
def test_daily_task_has_next_run(scheduler):
    now = datetime.now()
    task = ScheduledTask(
        id="sched_test5", name="每日任务", template="consolidate_memory",
        book_id="testbook", schedule_type="daily",
...
async def test_task_now_execution(scheduler):
    task = ScheduledTask(
        id="sched_test6", name="手动执行", template="review",
        book_id="testbook", schedule_type="manual",
        created_at=datetime.now().isoformat(), updated_at=datetime.now().isoformat(),
