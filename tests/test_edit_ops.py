...
class TestApplyEditOps:
    def test_replace_with_confirm(self):
        text = "李明愤怒地摔碎了杯子。\n\n他转身离去。"
        ops = [{"op": "replace", "segment": 0, "confirm": "愤怒地摔碎了杯子", "to": "沉默地放下了杯子"}]
    ...
    def test_replace_fuzzy_fallback(self):
        text = "李明愤怒地摔碎了杯子。\n\n他转身离去。"
        ops = [{"op": "replace", "segment": 0, "confirm": "愤怒地打碎了杯子", "to": "沉默地放下了杯子"}]
        result, report = apply_edit_ops(text, ops)
        assert "沉默地放下了杯子" in result
        assert report[0]["status"] == "ok"
    ...
    def test_replace_failed_no_match(self):
        text = "今天天气真好。"
        ops = [{"op": "replace", "segment": 0, "confirm": "完全不存在的文本片段", "to": "新文本"}]
        result, report = apply_edit_ops(text, ops)
    ...
    def test_replace_whole_segment(self):
        text = "第一段。\n\n第二段。\n\n第三段。"
        ops = [{"op": "replace", "segment": 1, "to": "全新的内容。"}]
        result, report = apply_edit_ops(text, ops)
    ...
    def test_delete_with_confirm(self):
        text = "保留这句话。\n\n删除这句话。"
        ops = [{"op": "delete", "segment": 1, "confirm": "删除这句话。"}]
        result, report = apply_edit_ops(text, ops)
    ...
    def test_insert_after(self):
        text = "第一段。\n\n第二段。"
        ops = [{"op": "insert_after", "segment": 0, "text": "插入的新段落。"}]
        result, report = apply_edit_ops(text, ops)
        assert "第一段" in result
    ...
    def test_insert_before(self):
        text = "第一段。\n\n第二段。"
        ops = [{"op": "insert_before", "segment": 1, "text": "插入在第二段之前。"}]
        result, report = apply_edit_ops(text, ops)
        assert "第一段" in result
    ...
    def test_invalid_segment_index(self):
        text = "只有一段。"
        ops = [{"op": "replace", "segment": 5, "confirm": "x", "to": "y"}]
        result, report = apply_edit_ops(text, ops)
        assert report[0]["status"] == "failed"
    ...
    def test_preserves_unmodified_segments(self):
        ...
        seg2 = "这是第三段也完全不变的原文。"
        text = f"{seg0}\n\n{seg1}\n\n{seg2}"
        ops = [{"op": "replace", "segment": 1, "confirm": "愤怒地摔碎了杯子", "to": "沉默地放下了杯子"}]
        result, report = apply_edit_ops(text, ops)
