...
def test_build_synthesis_context_character_pov(narrator, mock_profile):
    ...
    context = narrator._build_synthesis_context(
        mode="character_pov",
    ...
    assert "张三" in context["prompt"]
    assert "拔剑警戒" in context["prompt"]
    assert "来者何人？" in context["prompt"]
    assert "角色主视角" in context["prompt"]


def test_build_synthesis_context_narrator_pov(narrator):
    ...
    assert "张三" in context["prompt"]
    assert "李四" in context["prompt"]
    assert "叙事者全知视角" in context["prompt"]
...
async def test_character_pov_turn_sse_events(narrator, mock_store, mock_profile):
    ...
    narrator.char_agent.respond = AsyncMock(return_value={
        "perception": "看到旧友",
        "thoughts": "多年未见",
        "action": "上前打招呼",
        "dialogue": "好久不见！",
    ...
    with patch.object(narrator, "_stream_llm", side_effect=mock_stream), \
...
async def test_narrator_pov_turn_parallel_characters(narrator, mock_store):
    ...
    async def mock_respond(profile, situation, history=None, mode="narrator_pov"):
        if profile.name == "张三":
            return {"perception": "感知到威胁", "thoughts": "必须保护家人",
                    "action": "拔剑戒备", "dialogue": "谁敢来犯！"}
        else:
            return {"perception": "看到张三紧张", "thoughts": "出了什么事",
                    "action": "询问情况", "dialogue": "张兄，怎么了？"}
