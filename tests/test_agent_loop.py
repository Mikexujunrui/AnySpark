...
def test_unlimited_when_max_rounds_zero():
    """max_rounds=0 means unlimited — loop never exhausts via round count."""
    s = LoopState(max_rounds=0, base_temperature=0.3)
    for _ in range(500):
        s.advance_round()
    assert not s.exhausted
    assert not s.soft_limit_exceeded
