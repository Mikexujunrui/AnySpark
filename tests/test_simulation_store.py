...
def store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "core.simulation.simulation_store.DATA_DIR", tmp_path
    )
...
def test_add_and_get_events(store):
    """Test event CRUD."""
    session = store.create_session(mode="character_pov")
    e1 = store.add_event(session["id"], "第一段叙事", "narrative", 0)
    e2 = store.add_event(session["id"], "第二段叙事", "narrative", 1)
    events = store.get_events(session["id"])
    assert len(events) == 2
...
def test_add_and_get_choices(store):
    ...
    c1 = store.add_choice(event["id"], session["id"], "选项A", "后果A", "action")
    c2 = store.add_choice(event["id"], session["id"], "选项B", "后果B", "action")
