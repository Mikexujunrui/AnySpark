...
class TestPlannerDispatch:
    ...
    async def test_plan_write_new(self, tmp_data_dir, monkeypatch):
        ...
        monkeypatch.setattr(
            json_store, "load_outline", lambda bid: {"chapters": [{"title": "第一章"}, {"title": "第二章"}]}
        )
    ...
    async def test_plan_global_replace(self, tmp_data_dir, monkeypatch):
        ...
        monkeypatch.setattr(
            json_store, "load_chapters", lambda bid: [{"index": 1, "content": "x"}, {"index": 2, "content": "y"}]
        )
    ...
    async def test_plan_batch_edit(self, tmp_data_dir, monkeypatch):
        ...
        monkeypatch.setattr(
            json_store, "load_chapters", lambda bid: [{"index": i, "content": f"chapter {i}"} for i in range(1, 10)]
        )
        monkeypatch.setattr(json_store, "load_outline", lambda bid: {})
        monkeypatch.setattr(json_store, "load_detailed_outline", lambda bid: {})
    ...
    async def test_plan_targeted_edit(self, tmp_data_dir, monkeypatch):
        ...
        monkeypatch.setattr(json_store, "load_chapters", lambda bid: [{"index": 3, "title": "第三章", "content": "x"}])
        monkeypatch.setattr(json_store, "load_outline", lambda bid: {})
        monkeypatch.setattr(json_store, "load_detailed_outline", lambda bid: {})
    ...
    async def test_plan_analysis(self, tmp_data_dir, monkeypatch):
        ...
        monkeypatch.setattr(
            json_store, "load_chapters", lambda bid: [{"index": i, "content": f"ch{i}"} for i in range(1, 5)]
        )
        monkeypatch.setattr(json_store, "load_outline", lambda bid: {})
        monkeypatch.setattr(json_store, "load_detailed_outline", lambda bid: {})
...
class TestStepContextAccumulator:
    ...
    def test_record_and_retrieve(self):
        ...
        step = TaskStep(id="s1", type="agent_loop", label="规划第1章", config={"chapter_index": 1})
    ...
    def test_chapter_output_tracking(self):
        acc = StepContextAccumulator()
        step = TaskStep(id="s1", type="agent_loop", label="写作第1章", config={"chapter_index": 1})
    ...
    def test_build_context_injection_recent_steps(self):
        acc = StepContextAccumulator()
        for i in range(4):
            step = TaskStep(id=f"s{i}", type="agent_loop", label=f"步骤{i}", config={})
            acc.record(f"s{i}", step, {"text": f"结果{i}"})

        target_step = TaskStep(id="s4", type="agent_loop", label="下一步", config={})
    ...
    def test_build_context_injection_previous_chapter(self):
        ...
        step1 = TaskStep(id="s1", type="agent_loop", label="写作第1章", config={"chapter_index": 1})
        acc.record("s1", step1, {"text": "第1章的内容..."})

        # Build context for chapter 2
        step2 = TaskStep(id="s2", type="agent_loop", label="写作第2章", config={"chapter_index": 2})
    ...
    def test_build_context_injection_explicit_bindings(self):
        ...
        plan_step = TaskStep(id="plan_s0", type="agent_loop", label="规划", config={})
