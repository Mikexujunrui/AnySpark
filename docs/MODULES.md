...
| 依赖 | 知识图谱（SQLite Store）、全文搜索（SQLite FTS5） |
| 难度 | 困难 |
| 状态 | ✅ 已实现 |
...
| canon_purist | 原著党 | 专业 | 人设还原、世界观忠诚、时间线准确 |
| ai_flavor_sniffer | AI味嗅觉审查员 | 专业 | 机械表达、情感表演性、肢体逻辑、对话自然度 |
...
| character_voice_expert | 人物声音还原师 | 续写 | 角色语言风格还原、角色间区分度 |
| continuation_style_expert | 原著文风一致性 | 续写 | 文风一致性、叙事节奏对齐 |
| foreshadow_auditor | 伏笔回收审计员 | 续写 | 伏笔回收完整性、悬念处理 |
| annotation_compliance | 批注合规检察官 | 续写 | 批注线索遵循、高确定性约束检查 |
...
## 24. AI味扫描引擎 (AI Flavor Scanner)

| 属性 | 内容 |
|------|------|
| 层级 | 业务层 |
| 职责 | 7项纯规则检测AI写作痕迹，零token消耗，写后自动评分并反馈闭环 |
| 依赖 | 无（纯Python stdlib） |
| 状态 | ✅ 已实现 |
| 实现 | `src/core/ai_flavor_scanner.py` |

**7项检测:**
| 检测项 | 说明 | 阈值 |
|--------|------|------|
| AI过渡词密度 | 检测"与此同时/不仅如此"等机械过渡词 | >3/千字 |
| AI模板句式 | 检测"在这个充满……的世界里"等模板 | >2/千字 |
| AI模糊词密度 | 检测"似乎/仿佛/隐隐/莫名"等模糊词 | >5/千字 |
| 面部微表情密度 | 检测"眼眸/嘴角/眉头"等面部描写 | >8/千字 |
| 段落开头同构 | 连续段落以相同主语或时间词开头 | 连续3段 |
| 句长均匀度 | 句子长度标准差过小，缺少变化 | 标准差<8 |
| 对话标签多样性 | 检测标签单一性或花哨标签过度使用 | 多样性<0.2 |

**集成点:**
- `finalize_chapter` — 定稿时自动扫描
- `run_quality_gate` — 质量门评审前扫描
- `GET /books/{book_id}/ai-flavor-scan` — API端点
- `context_manager.py` — 反馈闭环注入
