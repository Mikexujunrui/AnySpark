...
| Phase 8: SQLite 重构 | ✅ 完成 | 2026-07 | Neo4j → SQLite 迁移 + 移除 Docker 依赖 |
...
- 描述: 知识图谱存储从 Neo4j 彻底迁移到嵌入式 SQLite（sqlite_store.py），零外部依赖，无需 Docker。删除所有 Docker 相关文件（docker-compose.yml, Dockerfile.backend, Dockerfile, nginx.conf, .dockerignore, install.sh, install.ps1）。移除 Neo4jConfig、Neo4jError 类。启动脚本简化，CI 移除 Neo4j 服务容器。更新所有文档和 .gitignore。
