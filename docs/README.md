...
| 🎨 全流程覆盖 | 🛡️ 工程级可靠 | 🔌 高度可扩展 |
|:---:|:---:|:---:|
| 从灵感构思 → 大纲规划 → 章节写作 → 评审修订 → 全书变换，一套工具走完全程 | 七层幻觉检测、Doom Loop 防护、Book 级写锁、指数退避重试——生产级 Agent 基础设施 | 插件系统、技能 YAML、文风 Profiles、自定义评审员——所有创作范式均可定制 |
...
| **七层幻觉检测** | 工具调用校验、序列型幻觉检测、确认陷阱识别、伪造工具调用检测等七层防护体系 |
...
| Python | >= 3.11 | 后端运行时 |
| Node.js | >= 20 | 前端构建 |
| Docker & Docker Compose | 最新稳定版 | Neo4j 数据库 + 一键部署 |
| DeepSeek API Key（或 OpenAI 兼容） | — | LLM 调用 |
...
> Docker 容器的 `data/` 目录映射到宿主机，请确保宿主机 `data/` 目录存在且有写权限。
...
- **Windows**：双击 `start.bat` 或 PowerShell 运行 `./start.ps1`
- **Mac / Linux**：`./start.sh`
