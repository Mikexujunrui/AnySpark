...
| routes_workspace | 工作区总览/章节 import/上传/上传区读取 + S141 AI 沙箱浏览（GET /api/sandbox 文件树 + GET /api/sandbox/file 读内容，read_file/write_file 产物人类可见） | workspace |
...
> 三 worker 只读侦察 + 主循环交叉核对。按严重度分级，处置需人工定夺。
