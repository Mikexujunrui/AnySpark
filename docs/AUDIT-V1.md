...
### ~~P0 — 知识图谱~~ ✅ **S7 已完成（commit `62246b1`）**：实体/关系/时间线事件 + FTS5 trigram、章节自动抽取、当前时空点注入、资料摘要卡图谱关联、检测图谱证据。
...
- **现状**：🟡 S7 已铺**图谱证据层**（`/api/check` 返回 `graph_evidence`：文本涉及的已知实体/关系供比对）；完整确定性规则（时间线顺序违反/伏笔匹配/章节结构校验）待补
