...
### 剩余（设计明确降权/后补，勿当缺失补）：
- **确定性校验完整规则**（时间线顺序违反/伏笔匹配/章节结构校验）——S7 已铺图谱证据层，规则化按需后补
- **关键点图谱**（T2 阶段 3）——DESIGN 标注"可选深入、可跳过"，降权项
- **网络搜索工具**（写作 Agent 侧）——智能体侧已有 websearch；Agent 内搜索按需后补
- **氛围滑块组/场景拼图板**（机制 4 其余组件）——能动性/候选卡/渐变条已落地，其余按需
- **L3 外部模式库**（机制 6）——明确按需后补（YAGNI）
