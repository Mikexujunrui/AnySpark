...
```bash
# 发布包（正式用户用，三平台自适应）：构建前端 → PyInstaller → 组装
bash scripts/build_release.sh            # 默认 v4.0.0
bash scripts/build_release.sh v4.1.0     # 指定版本
# 产物（平台自适应）：
#   Windows → AnySpark_Windows_x64_<v>.zip（AnySpark.exe 单文件）
#   macOS   → AnySpark_macOS_arm64_<v>.zip（AnySpark.app）
#   Linux   → AnySpark_Linux_x64_<v>.tar.gz（AnySpark 可执行）

# 源码分发目录（群内测/预览）：完整前后端源码 + 一键启动
uv run python scripts/package_release.py
```

> **自动发布（推荐）**：打 tag 即触发 CI 三平台自动构建并挂到 Release：
> `git tag v4.1.0 && git push --tags`（.github/workflows/release.yml）
>
> Release 页面只挂发布包（正式用户下载即用）；源码即本仓库，无需单独发布源码包。
