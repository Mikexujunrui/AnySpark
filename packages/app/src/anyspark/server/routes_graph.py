...
def make_graph_router(deps: AppDeps) -> APIRouter:
    ...
    def list_graph_types() -> list[dict[str, Any]]:
        ...
        return deps.graph.list_types("main")
    ...
    def patch_graph_entity(name_or_id: str, req: GraphEntityPatch) -> dict[str, Any]:
        ...
        ent = deps.graph.get_entity("main", name_or_id)
        if ent is None:
        ...
        ent = deps.graph.update_entity_fields("main", ent.name, **data)
        assert ent is not None
        return ent.to_dict()
    ...
    def delete_graph_entity(name_or_id: str) -> dict[str, bool]:
        ...
        if ent is None:
            raise HTTPException(status_code=404, detail=f"实体不存在: {name_or_id}")
        if not deps.graph.delete_entity("main", ent.name):
            raise HTTPException(status_code=404, detail=f"实体不存在: {name_or_id}")
        return {"ok": True}
    ...
    def graph_extract_route(req: GraphExtractIn) -> dict[str, int]:
        ...
        existing = [e.to_dict() for e in deps.graph.list_entities("main")]
        ...
        deps.graph.ingest_chapter("main", req.chapter_ref, order, ext)
        return {
