...
from anyspark.review.panel import _parse_review, _to_str_list, run_review
from anyspark.review.parse import extract_json, extract_scores, parse_review_json
...
def test_user_dir_overrides_system(tmp_path) -> None:
    panel = ReviewPanel()
...
def test_broken_yaml_does_not_crash(tmp_path) -> None:
    panel = ReviewPanel()
    bad = tmp_path / "bad"
    bad.mkdir()
...
def _panel_with(tmp_path, yaml_text: str) -> ReviewPanel:
    d = tmp_path / "rv"
    d.mkdir()
...
def test_run_review_happy_path(tmp_path) -> None:
...
def test_run_review_weighted_overall_wins(tmp_path) -> None:
...
def test_run_review_no_active_reviewers(tmp_path) -> None:
    panel = _panel_with(
        tmp_path,
        """
- id: off1
...
def test_run_review_all_failed(tmp_path) -> None:
    panel = _panel_with(tmp_path, SIMPLE_YAML)
...
def test_run_review_summarize_fallback(tmp_path) -> None:
...
def test_context_injection(tmp_path) -> None:
...
def test_specific_reviewer_selection(tmp_path) -> None:
    panel = _panel_with(
        tmp_path,
        SIMPLE_YAML
        + """
...
# ──────────────────────────────────────────────────────────────
def test_module_run_review(tmp_path) -> None:
    panel = _panel_with(tmp_path, SIMPLE_YAML)
    model = FakeModel(
        reviewer_responses={"测试评审员": _sample_reviewer_json()},
