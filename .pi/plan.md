...
| **SQLite 迁移功能缺陷**：① `_run` 对 Cypher 静默返回空；② SQLiteStore 缺方法；③ Row.get 崩溃 | ✅ 已完成（2026-07-31 专项收尾） | ① 22 处 Cypher 全部重写为 SQL/Python（`grep -rn "MATCH (" src/` 仅剩 base.py 检测逻辑）；② 20 个缺方法全部实现；③ `_run` 返回 dict；另修复 relations FK 过严（timeline/foreshadow 边原无法存储）——747 测试全绿 |
