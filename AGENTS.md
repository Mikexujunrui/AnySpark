...
### 注释纪律（S159 主人定案——S157 已清历史"主人/用户拍板"类注释，防回潮）

- **禁写"谁要求/谁拍板"**：注释里不得出现"主人/用户/老板要求""XX拍板/定案/指示"这类记录性措辞——git 历史已记录谁提的，注释不需要复述。
- **只写"为什么"**：注释回答三类问题——① 为什么这样设计（因果/动机/权衡）② 为什么这样处理（踩过的坑/实测证据/反例）③ 怎么用/注意什么（约束/边界/调用约定）。
- **阶段号可留、必须带因果**：`Sxxx：` 前缀保留可追溯，但后面必须跟因果说明；禁止只写 `（Sxxx：XX需求）` 这种空转注释。
- **不写废话**：能自明的代码不注释；注释解释"为什么"而非"是什么"；"实测"措辞保留（是设计依据证据，不是"谁要求"）。
- 现有"设计定案"类注释含因果内容的保留；纯"谁要求"类发现即清。
