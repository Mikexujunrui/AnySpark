...
async def lifespan(app: FastAPI):
    ...
    log_format = logging.Formatter(
        "%(asctime)s | %(levelname)-7s | %(name)-30s | %(message)s", datefmt="%Y-%m-%d %H:%M:%S"
    )
    ...
    # Writes to data/logs/connection.log with rotation.
    # This provides a clean signal of all SSE connection events for
    # diagnosing frontend-backend connectivity issues.
    conn_logger = logging.getLogger("connection.diagnostics")
    conn_logger.setLevel(logging.INFO)
    # Prevent propagation to root logger (avoids duplicate entries in server.log)
    conn_logger.propagate = False
    conn_file_handler = RotatingFileHandler(
        log_dir / "connection.log",
        maxBytes=10 * 1024 * 1024,  # 10MB
        backupCount=5,
        encoding="utf-8",
    )
    conn_file_handler.setFormatter(log_format)
    conn_logger.addHandler(conn_file_handler)
    # Also output to console for real-time monitoring in dev
    conn_console = logging.StreamHandler(sys.stdout)
    conn_console.setFormatter(log_format)
    conn_logger.addHandler(conn_console)

    logging.getLogger(__name__).info("Server starting up with centralized logging")
    conn_logger.info("[CONN-INIT] Connection diagnostics logger initialized | log=%s", log_dir / "connection.log")
...
# ── Production build: serve frontend static files ──
# In production, the Vite build output (frontend/dist) is served directly.
# In development, the Vite dev server on port 8190 is used instead.
# ── Frontend dist path ──
