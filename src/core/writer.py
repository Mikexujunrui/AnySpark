...
def _format_structure_constraints(structure: dict) -> str:
    ...
    if ch_count:
        lines.append(f"原著共 {ch_count} 章，{total} 字，平均每章 {avg:.0f} 字。")
    ...
    if para.get("avg_per_chapter"):
        lines.append(f"平均每章 {para['avg_per_chapter']:.0f} 段，每段约 {para['avg_length']:.0f} 字。")
    sent = structure.get("sentence_stats", {})
    if sent.get("avg_per_chapter"):
        lines.append(f"平均每章 {sent['avg_per_chapter']:.0f} 句，每句约 {sent['avg_length']:.0f} 字。")
...
def _format_style_constraints(fingerprint: dict) -> str:
    ...
    if para.get("mean"):
        lines.append(f"段落长度: 均值{para['mean']:.0f}字, 中位数{para.get('median', 0):.0f}字")
