...
def _classify_intent(instruction: str, book_state: dict) -> PlanIntent:
    ...
    return PlanIntent(
        ...
        requires_outline=bool(pattern.get("requires_outline", False)),
        requires_writing=bool(pattern.get("requires_writing", False)),
        requires_edit=bool(pattern.get("requires_edit", False)),
        requires_analysis=bool(pattern.get("requires_analysis", False)),
...
class AutopilotPlanner:
    ...
    async def _read_book_state(self, book_id: str) -> dict:
        ...
        return {
            ...
            "detailed_outline_count": len(detailed_chs)
            if detailed_outline and isinstance(detailed_outline, dict)
            else 0,
    ...
    def _plan_write_new(self, prefix: str, config: AutopilotConfig, book_state: dict, intent: PlanIntent) -> dict:
        ...
        for ch_info in chapters_to_write:
            ...
            ch_steps = self._build_chapter_steps(prefix, ch_idx, ch_title, config, step_counter)
    ...
    def _build_chapter_steps(
        self, prefix: str, ch_idx: int, ch_title: str, config: AutopilotConfig, offset: int
    ) -> list[TaskStep]:
    ...
    def _plan_batch_edit(self, prefix: str, config: AutopilotConfig, book_state: dict, intent: PlanIntent) -> dict:
        ...
        if intent.sequential_dependency:
            ...
            for ch_idx in target_indices:
                ...
                ch_steps = self._build_edit_steps(prefix, ch_idx, ch_title, config, intent.directive, step_counter)
                steps.extend(ch_steps)
                step_counter += len(ch_steps)
        else:
        ...
        return {
            "plan_summary": plan_summary,
            "chapters": [{"index": idx, "title": chapter_titles.get(idx, f"第{idx}章")} for idx in target_indices],
            "steps": steps,
    ...
    def _build_edit_steps(
        self, prefix: str, ch_idx: int, ch_title: str, config: AutopilotConfig, directive: str, offset: int
    ) -> list[TaskStep]:
    ...
    def _plan_global_replace(self, prefix: str, config: AutopilotConfig, book_state: dict, intent: PlanIntent) -> dict:
    ...
    def _plan_style_change(self, prefix: str, config: AutopilotConfig, book_state: dict, intent: PlanIntent) -> dict:
    ...
    def _plan_targeted_edit(self, prefix: str, config: AutopilotConfig, book_state: dict, intent: PlanIntent) -> dict:
        ...
        plan_summary = f"🎯 针对性精修模式：{' + '.join(parts)}「{intent.directive[:30]}」，共 {len(steps)} 个步骤。"
        if write_count:
    ...
    def _plan_analysis(self, prefix: str, config: AutopilotConfig, book_state: dict, intent: PlanIntent) -> dict:
        ...
        plan_summary = f"🔬 全书分析模式：执行「{intent.directive[:30]}」，先分析后根据结果动态规划修复步骤。"
    ...
    def _plan_insert_content(self, prefix: str, config: AutopilotConfig, book_state: dict, intent: PlanIntent) -> dict:
    ...
    async def _plan_import_and_refine(
        self, prefix: str, config: AutopilotConfig, book_state: dict, intent: PlanIntent
    ) -> dict:
        ...
        return {
            "plan_summary": plan_summary,
            "chapters": [{"index": ch["index"], "title": ch["title"]} for ch in to_import],
            "steps": steps,
    ...
    async def _plan_with_llm(self, prefix: str, config: AutopilotConfig, book_state: dict, intent: PlanIntent) -> dict:
...
- 只输出 JSON，不要加其他内容"""
    ...
    def _build_book_state_summary(self, book_state: dict) -> str:
        ...
        if book_state.get("outline_chapters"):
            ch_titles = [ch.get("title", f"第{i + 1}章") for i, ch in enumerate(book_state["outline_chapters"][:20])]
    ...
    async def replan(self, task: "PersistentTask", trigger_reason: str, accumulator=None) -> list[TaskStep]:
