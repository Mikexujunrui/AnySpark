    ...
    def _plan_targeted_edit(self, prefix: str, config: AutopilotConfig,
        ...
        for ch_idx in target_indices:
            ...
            steps.append(TaskStep(
                ...
                label=f"精修{ch_title}",
                config={
                    "prompt": f"根据分析报告，对{ch_title}进行精修。\n"
                              f"指令：「{intent.directive}」\n\n"
                              f"使用 patch_chapter 进行精确修改。"
        ...
        plan_summary = (
            f"🎯 针对性精修模式：对 {len(target_indices)} 个章节执行「{intent.directive[:30]}」，"
            f"共 {len(steps)} 个步骤（分析→精修→保存）。"
        )
