...
TAILSAFE_TOOLS = frozenset({
    "search_knowledge", "list_chapters", "read_chapter", "get_outline",
    "get_timeline", "get_detailed_outline", "get_worldbuilding",
    "chapter_history", "diff_chapters", "count_words", "list_volumes",
    "list_references", "list_reference_chapters", "search_reference",
    "get_style", "list_styles", "list_skills", "list_workflows",
    "browse_workflows", "browse_materials", "search_materials",
})
...
class LoopState:
    ...
    # round / retry accounting
    round: int = 0
    hallucination_retries: int = 0   # retries for fake_tool/fake_write (max 2)
    last_nudge_round: int = -100     # round when last wrap-up nudge was injected
    no_tool_nudges: int = 0          # nudges for premature terminal responses (no tool calls mid-task)
    ...
    def __post_init__(self) -> None:
        ...
        if not self.soft_round_limit:
            self.soft_round_limit = 30  # independent default, not tied to max_rounds
    ...
    def exhausted(self) -> bool:
        """Hard safety cap — only for runaway loops that fail to self-terminate."""
    ...
    def drift_limit_reached(self) -> bool:
        return (self.completion_claims >= COMPLETION_CLAIM_LIMIT
                or self.verify_loops >= VERIFY_LOOP_LIMIT)
