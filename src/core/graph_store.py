...
如果没有任何段落回收了该伏笔，输出 {{"found": false}}"""
    ...
    def _compute_character_importance(self) -> list[dict]:
        ...
        if not appear_map:
            appear_rows = self._run("""
                MATCH (t:Timeline {project_id: $pid})-[:INVOLVES]->(c:Entity:Character)
    ...
    def _compute_pacing_analysis(self) -> dict:
        ...
        events.sort(key=lambda e: e.time_order)
        max_order = int(max(e.time_order for e in events))

        density_map: dict[int, int] = {}
        for e in events:
            density_map[int(e.time_order)] = density_map.get(int(e.time_order), 0) + 1
    ...
    def _compute_character_heatmap(self) -> dict:
        co_rows = self._run("""
            MATCH (t:Timeline {project_id: $pid})-[:INVOLVES]->(c1:Entity:Character)
            MATCH (t)-[:INVOLVES]->(c2:Entity:Character)
            WHERE c1.id < c2.id
    ...
    def auto_complete_relations(self) -> dict:
        ...
        anom_rows = self._run("""
            ...
            OPTIONAL MATCH (t:Timeline {project_id: $pid})-[:INVOLVES]->(c)
...
- 每条建议必须给出简短理由"""
    ...
    def _compute_location_importance(self) -> list[dict]:
        ...
        event_rows = self._run("""
            MATCH (t:Timeline {project_id: $pid})-[:INVOLVES]->(l:Entity:Location {project_id: $pid})
    ...
    def _compute_organization_importance(self) -> list[dict]:
        ...
        event_rows = self._run("""
            MATCH (t:Timeline {project_id: $pid})-[:INVOLVES]->(o:Entity:Organization {project_id: $pid})
