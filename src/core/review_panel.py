...
SYSTEM_REVIEWERS_DIR = Path(__file__).parent.parent.parent / "reviewers"
# User reviewers: private custom data (gitignored, user backs up)
USER_REVIEWERS_DIR = Path(__file__).parent.parent.parent / "data" / "reviewers"
...
class ReviewPanel:
    ...
    def add_custom_reviewer(self, rid: str, name: str, persona: str,
                            category: str = "reader", avatar: str = "user",
                            scoring_dimensions: list[dict] | None = None,
                            needs_knowledge: bool = False) -> ReviewerDef:
        reviewer = ReviewerDef(
            id=rid,
            name=name,
            avatar=avatar,
    ...
    async def run_review(
        ...
        if mode == "serial":
            ...
            results, pending_tasks, pending_reviewers = (
                await self._run_concurrent(reviewers, chapter_text, knowledge_context, loop, executor, queue))
    ...
    async def _run_concurrent(self, reviewers, chapter_text, knowledge_context, loop, executor, queue):
        ...
        for r in reviewers:
            ...
            task = asyncio.ensure_future(
                self._single_review(r, chapter_text, ctx, loop, executor, queue))
            pending[task] = r
            ...
            for coro in done:
                ...
                try:
                    ...
                    result = ReviewResult(
                        ...
                        category=r.category, error="评审异常终止")
    ...
    async def _single_review(self, reviewer: ReviewerDef, chapter_text: str,
                             knowledge_context: str, loop, executor, queue) -> ReviewResult:
        if queue:
        ...
        return ReviewResult(
            ...
            category=reviewer.category, error=last_error,
        )
...
4. comment 用你的角色语气写，体现你的个性"""
    ...
    async def _summarize(self, results: list[ReviewResult], chapter_text: str,
                         book_id: str, chapter_ref: str, loop, executor) -> ReviewReport:
        report = ReviewReport(book_id=book_id, chapter_ref=chapter_ref)
        ...
        if not valid:
            ...
            report.individual_reviews = [
                {"reviewer": r.reviewer_name, "error": r.error} for r in results
            ]
            return report
