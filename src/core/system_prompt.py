...
def build_system_prompt(agent_type: str = "write", style_name: str = "", **kwargs) -> str:
    ...
    if agent_type == "plan" and "{PLAN_DISABLED_TOOLS}" in base_prompt:
        disabled = sorted(
            ...
            if meta.get("mutates_kb") or meta.get("touches_chapter")
        )
        # 额外工具不在 TOOL_META 中但仍需禁用
        extra = {"delete_version", "purge_chapter_history", "manage_volumes",
                 "add_material", "delete_material", "delete_timeline_event",
                 "delete_foreshadow", "prepare_writing", "finalize_chapter"}
