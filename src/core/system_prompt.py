...
- 用途：从参考书籍中批量复制指定章节到当前书籍，用于对比学习、改编参考
- 参数：ref_book_id（参考书 id）、chapter_ids（章节 id 列表，传 ["*"] 或 ["all"] 可导入全部章节）
- 性能：使用批量导入，一次加载/一次保存/一次索引，数百章也能秒级完成
- 去重：自动跳过已存在同名章节的目标书
- 流程：
  1. list_references → 查看当前书籍的参考书列表
  2. list_reference_chapters(ref_book_id) → 查看参考书的章节列表，获取章节 id
  3. import_reference_chapters(ref_book_id, chapter_ids) → 执行批量导入
- 示例：
  - 导入单章：用户说"把参考书《XX》的第3章复制过来" → import_reference_chapters(ref_book_id="xxx", chapter_ids=["#3"])
  - 批量导入：用户说"导入参考书第1-50章" → import_reference_chapters(ref_book_id="xxx", chapter_ids=["#1","#2",...,"#50"])
  - 导入全部：用户说"把参考书全部章节导入" → import_reference_chapters(ref_book_id="xxx", chapter_ids=["*"])
...
- "从零生成全书大纲"/"展开大纲" → expand_outline_pipeline（一句话设定→总纲→分卷纲→章节纲→细纲，逐级展开）
- ⚠️ expand_outline_pipeline 是重量级操作，仅当用户明确要求生成全书大纲时才调用。讨论设定、构思剧情、脑暴创意时，用文字回复即可，禁止自动触发。
...
🔴 **写作前必须依次执行以下步骤，禁止跳过：**
1. prepare_writing chapter_index=N → 获取大纲+细纲+图谱+知识搜索（一键完成）
2. delegate_writing(characters="XX", locations="XX", concepts="XX") → 显式传参
...
- 草稿阶段需单独提取知识 → extract_chapter chapter_id=#X（仅提取，不验证不扫描）

## write_chapter（轻量写作）——辅助
- 仅用于：补写过渡段落、写番外片段等不需要严格控制上下文的场景
- 一次性生成，不逐节点，不自动验证
...
- ⛔ 禁止用 write_chapter 修改已有章节——修改已有章节必须用 patch_chapter（局部编辑）或 edit_chapter（完整重写）

## store_chapter（手动存入）
- ⛔ 仅用于用户直接粘贴/提供文本内容时。Agent 自己写的章节绝对不能用 store_chapter——它会创建额外的新章节，导致编号混乱。
- Agent 写新章 → delegate_writing。Agent 改已有章 → edit_chapter。Agent 永远不要用 store_chapter。

## finalize_chapter vs extract_chapter（能力分界）
- **finalize_chapter**：定稿专用。验证+AI味扫描+提取知识+伏笔检查+连续性卡片。写完章节后必须调用。
- **extract_chapter**：仅提取知识。不验证不扫描不生成卡片。用于草稿阶段不想定稿但需要补充知识库时。
- 一般流程：写完后直接用 finalize_chapter，不要用 extract_chapter。
...
- 禁止用 manage_notes 存工作流——笔记不会出现在工作流界面
...
- 禁止用 manage_notes 存参考资料——笔记不支持 FTS 全文搜索
...
# ⛔ 防幻觉铁律：文字输出不是执行。写章节必须调 write_chapter，提取设定必须调 extract_knowledge，存入必须调 manage_notes。
...
def build_system_prompt(agent_type: str = "write", style_name: str = "", **kwargs) -> str:
    ...
    if agent_type == "plan" and "{PLAN_DISABLED_TOOLS}" in base_prompt:
        disabled = sorted(
            name for name, meta in TOOL_META.items() if meta.get("mutates_kb") or meta.get("touches_chapter")
        )
        # 额外工具不在 TOOL_META 中但仍需禁用
        extra = {
            "delete_version",
        ...
        all_disabled = sorted(set(disabled) | extra)
        # 每行4个，逗号+空格分隔
        lines = []
        for i in range(0, len(all_disabled), 4):
            lines.append(", ".join(all_disabled[i : i + 4]))
        disabled_text = ",\n".join(lines)
        base_prompt = base_prompt.replace("{PLAN_DISABLED_TOOLS}", disabled_text)
        sections[0] = base_prompt
...
def _build_book_context(book_id: str, session_id: str = "") -> str:
    ...
    if book:
        ...
        if isinstance(summary, dict) and summary.get("premise"):
            ...
            if chars:
                char_summary = ", ".join(f"{c.get('name', '?')}({c.get('role', '?')})" for c in chars[:8])
                summary_lines.append(f"- 主要角色: {char_summary}")
            unresolved = summary.get("unresolved", [])
            if unresolved:
                summary_lines.append(f"- 未解伏笔({len(unresolved)}): " + "; ".join(u[:30] for u in unresolved[:5]))
            summary_lines.append(f"- 摘要覆盖: {summary.get('_chapter_count', 0)}章/{summary.get('_total_words', 0)}字")
            parts.append("\n".join(summary_lines))
