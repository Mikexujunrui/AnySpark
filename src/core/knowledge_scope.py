...
class WritingKnowledgeScope:
    ...
    # ── 深度文风约束（由 reference_analyzer/emotion_analyzer 生成）──
    # 以下字段默认为 None 表示"未启用"。仅在设置这些字段时，
    # context_manager 才会将其注入写作上下文。
    sentence_rhythm: dict | None = None
    rhetoric_density: dict | None = None
    prophecy_signature: dict | None = None
    narrative_pov: dict | None = None
    emotional_curve: dict | None = None

    # ── 显式功能开关（默认为 False，需显式启用）──
    inject_voice_constraints: bool = False  # 是否注入角色声音约束
    inject_annotation_constraints: bool = False  # 是否注入批注/评点约束
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "rhetoric_density": self.rhetoric_density,
            "prophecy_signature": self.prophecy_signature,
            "narrative_pov": self.narrative_pov,
            "emotional_curve": self.emotional_curve,
            # 功能开关
            "inject_voice_constraints": self.inject_voice_constraints,
            "inject_annotation_constraints": self.inject_annotation_constraints,
        }
    ...
    def to_summary(self) -> str:
        ...
        if self.structure_report:
            ...
            lines.append(f"\n## 原著结构\n{ch} 章, 平均 {avg} 字/章")
        if self.sentence_rhythm is not None and len(self.sentence_rhythm) > 0:
            lines.append(f"\n## 句式韵律\n{len(self.sentence_rhythm)} 项指标")
        if self.rhetoric_density is not None and len(self.rhetoric_density) > 0:
            lines.append(f"\n## 修辞密度\n{len(self.rhetoric_density)} 项指标")
        if self.prophecy_signature is not None and len(self.prophecy_signature) > 0:
            lines.append(f"\n## 谶语特征\n{len(self.prophecy_signature)} 项指标")
        if self.narrative_pov is not None and len(self.narrative_pov) > 0:
    ...
    def from_dict(cls, d: dict) -> "WritingKnowledgeScope":
        return cls(
            ...
            rhetoric_density=d.get("rhetoric_density", {}),
            prophecy_signature=d.get("prophecy_signature", {}),
            narrative_pov=d.get("narrative_pov", {}),
            emotional_curve=d.get("emotional_curve", {}),
        )
