...
def _extract_quality_stream(text: str, existing_knowledge: str, book_id: str):
    ...
    yield {
        "event": "progress",
        "data": {"stage": "分析完成", "detail": f"识别 {len(result.entities)} 个实体, {len(result.relations)} 条关系"},
    }
...
def _extract_split_stream(text: str, existing_knowledge: str, book_id: str):
    ...
    yield {
        "event": "progress",
        "data": {
            "stage": "Step 0: NER pre-scan",
            "detail": "Running spaCy NER to pre-identify entities" if _NER_AVAILABLE else "（NER模块未加载，跳过）",
        },
    }
    ...
    yield {
        "event": "progress",
        "data": {
            "stage": f"Step 1 done: {total} 个实体",
            "detail": f"发现 {len(rough.relations)} 条关系, {len(rough.foreshadows)} 个伏笔 — 开始并行提取属性",
            "found": total,
        },
    }
    merged.relations = rough.relations
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        ...
        for future in as_completed(futures):
            ...
            yield {
                "event": "progress",
                "data": {
                    "stage": f"Step 2: 提取属性 ({completed}/{total})",
                    "detail": f"已完成 {entity.name}",
                    "done": completed,
                    "total": total,
                },
            }
    ...
    yield {
        ...
        "data": {
            ...
            "detail": f"{len(merged.entities)} entities, {len(merged.relations)} relations, {len(merged.foreshadows)} foreshadows",
        },
    }
    yield {"event": "result", "data": _proposal_to_dict(merged)}
...
def _proposal_to_dict(p: KnowledgeProposal) -> dict:
    return {
        "entities": [
            {"id": e.id, "type": e.type, "name": e.name, "aliases": e.aliases, "data": e.data} for e in p.entities
        ],
...
def accept_proposal(proposal: KnowledgeProposal, project_id: str = "default") -> str:
    ...
    if (
        ac.get("symmetry_added")
        or ac.get("paired_added")
        or ac.get("unidirectional_cleaned")
        or ac.get("cooccur_added")
        or ac.get("transitive_added")
        or ac.get("structural_added")
        or ac.get("llm_suggested")
        or ac.get("multihop_added")
        or ac.get("jaccard_added")
    ):
        parts = []
    ...
    bus.emit_sync(
        Event(
            ...
            data={
                ...
                "foreshadows": len(proposal.foreshadows),
            },
        )
    )
