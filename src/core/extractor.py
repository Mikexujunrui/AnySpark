...
EXTRACTION_SYSTEM = """你是一位专门分析小说文本的知识提取专家。你的任务是从作者提供的任意文本中，按照严格的结构化模板提取所有可识别的世界观设定。

# 提取原则
1. **严格按模板填空**：每种实体类型都有固定的字段结构，请按模板字段逐一提取
2. **只提取文中明确出现的信息**：文中没提到的字段留空即可，绝不编造
3. **属性不求全但求细**：哪怕只提到一句话，也要拆解到对应字段
4. **注意隐含信息**：如"他曾是剑帝"意味着隐藏身份为前任剑帝，"黑袍遮面"意味着外貌特征为黑袍斗篷遮面
5. **注意对话风格**：角色的自称（"本座""老子""妾身"）、口头禅、对不同人的称呼方式，都是重要的角色特征。遇到角色对话时特别留意提取"对话"区块的字段
6. **识别新旧实体**：查看"已有实体列表"，如果文中提到的角色/地点等已存在，在 entity 对象中加入 "existing_id" 字段标注
7. **同一实体只出现一次**：不要在 entities 数组中重复提取同一个实体
...
# 关系类型
knows（认识）, belongs_to（属于）, located_at（位于）, owns（拥有）, antagonist（敌对）,
ally（同盟）, family（家族/血缘）, romantic（恋爱）, master_of（师徒/上下级）, causes（导致）,
before（时间先于）, after（时间后于）, foreshadows（伏笔）, resolves（回收伏笔）, participates_in（参与）
...
{{
  "entities": [
    {{
      ...
      "name": "姓名",
      "aliases": ["别名1"],
      "existing_id": "已有实体ID（如果匹配已有实体则填写，否则不填）",
      "data": {{
        "基本": {{ "name": "...", "age": "18", ... }},
        "外貌": {{ "appearance": "...", "eyes": "蓝色" }},
        "性格": {{ "personality": "...", "motivation": "..." }},
        "能力": {{ "cultivation_level": "..." }},
        "背景": {{ "origin": "...", "background": "..." }},
        "状态": {{ "current_location": "..." }},
        "对话": {{ "speaking_style": "...", "self_address": "本座" }}
      }}
    }}
  ],
  "relations": [
    {{ "from": "实体名", "to": "实体名", "type": "关系类型", "data": {{}} }}
  ],
  "foreshadows": [
    {{ "text": "原文暗示语句", "hint": "暗示了什么", "expected_resolution": "可能的后续" }}
  ]
}}

data 中的每个一级key对应上述模板中的区块名（如"基本"、"外貌"、"性格"），区块内的key对应模板中的字段名。
文中未提及的字段不填（不要放空字符串）。"""
...
def _extract_quality_mode(text: str, existing_knowledge: str, book_id: str) -> KnowledgeProposal:
    ...
    system = EXTRACTION_SYSTEM.replace("{schemas}", schema_text)
...
def _ner_augmented_scan(text: str, existing_knowledge: str, book_id: str) -> KnowledgeProposal:
    ...
    pass1_system = """你是一位小说文本快速扫描助手。只需识别文中出现了哪些实体，不要求详细属性。
关系类型必须是以下之一: ally（同盟）, antagonist（敌对）, family（家族/血缘）, romantic（恋爱）, master_of（师徒/上下级）, owns（拥有）, located_at（位于）, belongs_to（属于）, causes（导致）, participates_in（参与）, knows（认识）。
输出严格的 JSON：
{"entities": [
  {"type": "character", "name": "姓名", "aliases": ["别名1"]},
  {"type": "location", "name": "地名"},
  {"type": "item", "name": "物品"},
  {"type": "organization", "name": "组织"},
  {"type": "concept", "name": "概念"},
  {"type": "event", "name": "事件"}],
 "relations": [{"from": "A", "to": "B", "type": "ally"}],
 "foreshadows": [{"text": "暗示语句", "hint": "暗示了什么", "expected_resolution": "可能后续"}]
}

只列明确出现的实体，不填充细节属性。不要输出无关解释。"""
...
def _extract_batch(entities: list, text: str) -> list[Entity]:
    ...
    system = f"""你是小说知识提取专家。一次提取多个实体的详细属性，每个实体独立输出。

各类型模板：
{schema_text}

只提取原文明确提到的信息，不编造。输出严格 JSON：
{{"entities": [
  {{"name": "实体名", "type": "character", "data": {{"基本": {{...}}, "外貌": {{...}} }} }},
  ...
]}}
每个实体的 data 中只包含该类型模板中的字段名。没有的字段不填（不要放空字符串）。"""
...
def _extract_split_stream(text: str, existing_knowledge: str, book_id: str):
    ...
    pass1_system = """You are a novel text scanner. Identify all entities and their relationships.
Relationship types MUST be one of: ally, antagonist, family, romantic, master_of, owns, located_at, belongs_to, causes, participates_in, knows.
Output JSON: {"entities":[{"type":"character","name":"name"}],"relations":[{"from":"A","to":"B","type":"one_of_above_types"}],"foreshadows":[{"text":"hint text","hint":"what it hints","expected_resolution":"possible reveal"}]}"""
    pass1_prompt = ""
    if existing_knowledge:
        kb_summary = _summarize_entities(existing_knowledge, book_id)
        if kb_summary:
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.warning(f"Entity update task failed for {entity.name}: {e}")
            completed += 1
...
def _parse_proposal(response: str) -> KnowledgeProposal:
    ...
    return KnowledgeProposal(entities=entities, relations=relations, foreshadows=foreshadows)
...
def accept_proposal(proposal: KnowledgeProposal, project_id: str = "default") -> str:
    ...
    try:
        ...
        logger.warning(f"FTS index_entity batch failed: {e}")
