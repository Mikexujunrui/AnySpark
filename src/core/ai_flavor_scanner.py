...
def _density_check(
    ...
    return FlavorCheck(
        name=name, passed=passed, score=min(100.0, score),
        threshold=threshold, actual_value=actual, details=details,
    )
...
def _check_ai_transition_words(content: str, total_chars: int, threshold: float) -> FlavorCheck:
    return _density_check(
        "AI过渡词密度", _TRANSITION_RE, content, total_chars, threshold,
        "AI过渡词密度",
    )
...
def _check_ai_template_patterns(content: str, total_chars: int, threshold: float) -> FlavorCheck:
    ...
    return FlavorCheck(
        name="AI模板句式密度", passed=passed, score=min(100.0, score),
        threshold=threshold, actual_value=actual, details=details,
    )
...
def _check_ai_hedge_words(content: str, total_chars: int, threshold: float) -> FlavorCheck:
    return _density_check(
        "AI模糊词密度", _HEDGE_RE, content, total_chars, threshold,
        "AI模糊词密度",
    )
...
def _check_facial_micro_expressions(content: str, total_chars: int, threshold: float) -> FlavorCheck:
    return _density_check(
        "面部微表情密度", _FACIAL_RE, content, total_chars, threshold,
        "面部微表情密度",
    )
...
_SAME_SUBJECT_START = re.compile(
    r"^(他|她|它|他们|她们|.{1,2})(?:也|又|便|就|却|还|只|才|已|都|总|再)"
)

_TIME_OPENING_WORDS = [
    "此时", "这时", "片刻之后", "片刻后", "那一刻", "不一会儿",
    "没多久", "很快", "紧接着", "下一秒", "下一瞬", "刹那间",
]
...
def _check_paragraph_openings(content: str, threshold: int = 3) -> FlavorCheck:
    ...
    if len(paragraphs) < threshold:
        return FlavorCheck(
            name="段落开头同构", passed=True, score=100.0,
            threshold=float(threshold), actual_value=0.0, details=[],
        )
    ...
    for i in range(1, len(paragraphs)):
        ...
        same_subject = (
            prev_subj and curr_subj
            and prev_subj.group(1) == curr_subj.group(1)
        )

        same_time = any(
            curr.startswith(tw) and prev.startswith(tw)
            for tw in _TIME_OPENING_WORDS
        )
        ...
        if same_subject or same_time:
            ...
            if current_consecutive >= threshold:
                issues.append(
                    f"第{i - current_consecutive + 1}~{i}段"
                    f"连续以相同模式开头（共{current_consecutive}段）"
                )
    ...
    if current_consecutive >= threshold:
        ...
        issues.append(
            f"第{start_idx + 1}~{len(paragraphs)}段"
            f"连续以相同模式开头（共{current_consecutive}段）"
        )
    max_consecutive = max(max_consecutive, current_consecutive)
...
def _check_sentence_length_uniformity(
    content: str, threshold: float = 8.0,
) -> FlavorCheck:
    """Check if sentences are too uniform in length (AI hallmark)."""
    sentences = _split_sentences(content)
    if len(sentences) < 3:
        return FlavorCheck(
            name="句长均匀度", passed=True, score=100.0,
            threshold=threshold, actual_value=0.0, details=[],
        )
    ...
    return FlavorCheck(
        name="句长均匀度", passed=passed, score=max(0.0, score),
        threshold=threshold, actual_value=round(std_dev, 1), details=issues,
    )
...
_FANCY_TAG_PATTERNS = [
    "低语道", "轻笑道", "冷声道", "厉声道", "柔声道", "朗声道",
    "微微笑道", "低声道", "高声道", "沉声道", "淡淡道", "轻轻道",
    "冷冷道", "苦笑道", "冷笑道", "淡笑道", "浅笑道", "嗤笑道",
    "淡笑", "浅笑", "嗤笑", "轻笑", "低语", "喃喃",
]


def _check_dialogue_tag_monotony(
    content: str, threshold: float = 0.2,
) -> FlavorCheck:
    """Check dialogue tag diversity and fancy-tag overuse."""
    dialogues = _extract_dialogues(content)
    if len(dialogues) < 3:
        return FlavorCheck(
            name="对话标签多样性", passed=True, score=100.0,
            threshold=threshold, actual_value=0.0, details=[],
        )
    ...
    if diversity < threshold:
        issues.append(
            f"对话标签多样性 {diversity:.2f}（阈值 {threshold:.2f}），"
            f"仅{unique_tags}种/共{total_tags}个"
        )
    ...
    return FlavorCheck(
        name="对话标签多样性", passed=passed, score=max(0.0, score),
        threshold=threshold, actual_value=round(diversity, 3), details=issues,
    )
...
def scan_chapter_custom(
    ...
    if not content or not content.strip():
        return AIFlavorReport(
            overall_score=100.0, is_clean=True,
    ...
    check_map: dict[str, tuple[Callable, float]] = {
        key: (factory, t[key]) for key, factory in _CHECK_SPECS
    }
    ...
    for key in all_checks:
        ...
        try:
            ...
            checks.append(FlavorCheck(
                name=key, passed=True, score=100.0,
                threshold=t.get(key, 0), actual_value=0,
                details=["检测异常，跳过"],
            ))
