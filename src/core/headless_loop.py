...
async def run_agent_loop_headless(
    ...
    if final_text:
        _persist_headless_turn(book_id, session_id, instruction, final_text,
                               mode="write", parts=turn_parts, source=source)
...
class TaskRunner:
    ...
    async def start_task(self, task_id: str) -> bool:
        ...
        await bus.emit(Event(
            ...
            data={"task_id": task_id, "book_id": task.book_id,
                  "label": task.label, "total_steps": len(task.steps)},
            source="task_runner",
        ))
        ...
        try:
            while loop_count < MAX_LOOPS:
                ...
                if skip_requested:
                    ...
                    if step:
                        self._queue.mark_step_completed(task_id, step.id,
                            {"skipped": True, "reason": "用户跳过"})
                        self._queue.advance_step(task_id, {"skipped": True})
                        await bus.emit(Event(
                            type=EventType.TASK_STEP_COMPLETED,
                            data={"task_id": task_id, "step_id": step.id,
                                  "step_label": step.label, "result": {"skipped": True}},
                            source="task_runner",
                        ))
                    continue
                ...
                if step is None:
                    ...
                    await bus.emit(Event(
                        ...
                        data={"task_id": task_id,
                              "progress": self._queue.get_progress(task_id)},
                        source="task_runner",
                    ))
                    return
                ...
                try:
                    ...
                    await bus.emit(Event(
                        type=EventType.TASK_STEP_COMPLETED,
                        data={"task_id": task_id, "step_id": step.id,
                              "step_label": step.label, "result": result or {}},
                        source="task_runner",
                    ))
                    ...
                    if step.config.get("trigger_replan"):
                        replan_msg = f"分析步骤 '{step.label}' 已完成，分析结果: {str(result.get('text', result))[:300]}"
                    ...
                    await bus.emit(Event(
                        ...
                        data={"task_id": task_id, "step_id": step.id,
                              "step_label": step.label, "error": error_msg},
                        source="task_runner",
                    ))
                    ...
                    if current_step and current_step.id == step.id:
                        if step.retry_count < step.max_retries:
                            logger.info("Retrying step %s (attempt %d/%d)",
                                        step.id, step.retry_count + 1, step.max_retries)
                ...
                if step.config.get("final"):
                    ...
                    pause_seconds = (persistent_task.metadata or {}).get("pause_between_chapters", 0) if persistent_task else 0
            ...
            if loop_count >= MAX_LOOPS:
                logger.error("Task %s exceeded max loops (%d), marking as failed",
                             task_id, MAX_LOOPS)
                self._queue.update_task_status(
                    task_id, TaskStatus.FAILED,
                    error=f"Exceeded maximum execution loops ({MAX_LOOPS})")
            ...
            self._queue.update_task_status(
                task_id, TaskStatus.FAILED, error=str(e)[:300])
        finally:
            self._running.pop(task_id, None)
            self._cancel_flags.pop(task_id, None)
    ...
    async def _execute_step(self, task_id: str, step) -> dict:
        ...
        if step_type == "agent_loop":
            ...
            await bus.emit(Event(
                type=EventType.TASK_NOTIFICATION,
                data={"task_id": task_id, "question_id": q_req.id,
            ...
            try:
                answers = await asyncio.wait_for(
                    question_manager.wait_for_answer(q_req.id), timeout=600)
    ...
    async def _maybe_replan(self, task_id: str, step, error_msg: str,
                              accumulator: "StepContextAccumulator") -> bool:
        ...
        logger.info("Task %s: triggering replan (step %s failed: %s)",
                     task_id, step.label, error_msg[:100])

        try:
            from .autopilot import AutopilotPlanner
            planner = AutopilotPlanner()
            new_steps = await planner.replan(
                task, f"步骤 '{step.label}' 失败: {error_msg[:200]}", accumulator
            )
            ...
            if self._queue.replace_remaining_steps(task_id, new_steps):
                ...
                self._queue.update_task_meta(task_id, {
    ...
    async def recover_pending_tasks(self):
        ...
        logger.info("Recovered %d running + %d pending tasks",
