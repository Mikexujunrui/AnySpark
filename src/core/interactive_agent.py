"""[DEPRECATED] Interactive Scenario Agent — generates narrative segments with choices.

.. deprecated:: 2.0
    This module is superseded by ``core.simulation.narrator_agent.NarratorAgent``
    and ``core.simulation.character_agent.CharacterAgent``. The new architecture
    uses a dual-agent model (Narrator + Character) with SSE streaming and
    graph-driven option generation.
