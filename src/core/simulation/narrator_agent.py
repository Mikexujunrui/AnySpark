...
class NarratorAgent:
    ...
    async def start(
        ...
        config_preview = {
            ...
            "graph_insights": {
                "forgotten_characters": [c.get("name", "") for c in insights.get("forgotten_characters", [])[:5]],
    ...
    async def _stream_llm(self, prompt: str, system: str, temperature: float = 0.8) -> AsyncGenerator[str, None]:
                    ...
                    if wb and isinstance(wb, dict):
                        ...
                        if isinstance(entries, list) and entries:
                            parts.append(
                                ...
                                + "; ".join(str(e.get("name", e))[:80] for e in entries[:5] if isinstance(e, dict))
                            )
                except Exception:
                    continue
