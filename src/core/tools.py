...
class Tool:
    ...
    # Extended usage documentation — richer than ``description``, used by
    # ``to_doc()`` to generate the tool-usage section of the system prompt.
    # When set, ``build_tool_docs()`` picks it up automatically so you no
    # longer need to maintain the same docs in ``system_prompt.py``.
    doc: str = ""
    ...
    def to_doc(self) -> str:
        """Return an extended documentation section for this tool.

        Falls back to ``description`` when ``doc`` is empty.  The output is
        markdown suitable for inclusion in a system prompt.
        """
        if self.doc:
            body = self.doc
        else:
...
def validate_tool_input(tool: Tool, args: dict) -> tuple[dict, list[str]]:
    ...
    Uses Pydantic-based validation (via ``validate_with_pydantic``) as the
    primary engine, falling back to the legacy manual type-coercion logic
    when dynamic model generation fails.
    """
...
registry.register(Tool(
    ...
    parameters={
        "entry_id": {"type": "string", "description": "条目ID，从 get_worldbuilding 获取"},
        "data": {"type": "object", "description": "要更新的字段，如 {\"title\": \"新标题\", \"content\": \"新内容\", \"tags\": [\"标签1\"]}"},
    },
    dangerous=True,
))
...
registry.register(Tool(
    ...
    parameters={
        "foreshadow_id": {"type": "string", "description": "伏笔ID，从 search_knowledge 或 list_pending_foreshadows 获取"},
        "resolution_text": {"type": "string", "description": "回收说明，描述伏笔如何被回收、在哪个章节的什么情节中实现"},
    },
))
...
registry.register(Tool(
    ...
    description="批量编辑多个章节的全部内容。传入章节ID数组和替换内容，一次性覆盖多个章节。适用于全书范围的批量改写。",
    parameters={
        "chapter_ids": {"type": "array", "items": {"type": "string"}, "description": "章节ID数组，如 ['#1', '#3', '#5']"},
        "content": {"type": "string", "description": "替换后的章节内容（所有指定章节使用相同内容）"},
...
registry.register(Tool(
    ...
    doc=(
        ...
        "1. 【推荐】prepare_writing chapter_index=N → 一键获取大纲+细纲+知识搜索+图谱洞察\n"
        "2. 或手动: get_outline → get_detailed_outline → search_knowledge\n"
        "3. delegate_writing(characters='角色A', locations='地点X', concepts='设定Y') → 显式传参\n\n"
        ...
        "**逐节点写作**：有细纲时自动按 plot_chain 逐节点写作（每节点默认350字），"
        "无细纲时回退到一次性写作。\n"
        "可指定 target_words_per_node 控制每节点字数。\n"
        "无细纲时先 generate_detailed_outline 生成，再 delegate_writing。\n"
        "写完后自动运行 verify_chapter 验证：实体漂移/大纲合规/约束/伏笔/可信度。\n\n"
        "**番外**：写番外用 is_extra=true。番外大纲同样包含 synopsis/characters/notes，写作时自动注入。"
    ),
    parameters={
...
registry.register(Tool(
    ...
    parameters={
        ...
        "keywords": {
            "type": "string", "description": "偏好条目的关键词（仅 target=preference 时使用），逗号分隔",
            "required": False,
        },
        "confidence": {
            "type": "string", "description": "偏好条目的置信度（仅 target=preference 时使用）: confirmed(已确认) | pending(待确认)",
