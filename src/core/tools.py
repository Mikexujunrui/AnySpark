...
registry.register(Tool(
    ...
    parameters={
        ...
        "tags": {"type": "array", "items": {"type": "string"}, "description": "标签（action=add时可选）", "required": False},
        "note_id": {"type": "string", "description": "笔记ID（action=delete时必填，从list获取）", "required": False},
    },
))
...
registry.register(Tool(
    ...
    parameters={
        "order": {"type": "array", "items": {"type": "string"}, "description": "章节 ID 列表，按新顺序排列（如 ['id1','id3','id2']）"},
    },
    dangerous=True,
))
