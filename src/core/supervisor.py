...
class Supervisor:
    ...
    def start(self):
        ...
        if self._loop_task is None or self._loop_task.done():
            self._loop_task = asyncio.create_task(self._supervision_loop(), name="supervisor")
    ...
    async def _check_stale_tasks(self):
        ...
        for task in running_tasks:
            ...
            try:
                ...
                if last_dt < cutoff:
                    logger.warning("Stale task %s: no progress for %s", task.id, now - last_dt)
                    self._notify(
                        task,
        ...
        for task in failed_tasks:
            ...
            if count >= self._config.max_auto_retries:
                ...
                if count == self._config.max_auto_retries:
                    self._notify(
                        task,
                        f"任务 {task.label} 的步骤 '{failed_step.label}' 已重试{count}次仍然失败，需要人工介入",
                    )
