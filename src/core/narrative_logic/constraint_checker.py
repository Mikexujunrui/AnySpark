...
Read-only: only runs MATCH / RETURN queries.  Custom Cypher fragments
are sanitized to reject any write operations (CREATE / DELETE / SET /
MERGE / REMOVE / DROP).
"""
...
# Cypher keywords that indicate a write operation — rejected for safety.
_WRITE_KEYWORDS = re.compile(
    r"\b(CREATE|DELETE|SET|MERGE|REMOVE|DROP|CALL)\b",
    re.IGNORECASE,
)
...
class ConstraintChecker:
    ...
    def check_one(self, constraint: Constraint) -> ConstraintViolation | None:
        ...
        rows = self._store._run(constraint.violation_query, {
            "pid": self._store.project_id,
        })
        violations = []
        for row in rows:
            # Flatten the row into a simple dict
            entry = {}
            for key in row.keys():
                val = row[key]
                # Neo4j nodes need .name extraction
                if hasattr(val, "get") and not isinstance(val, str):
                    entry[key] = val.get("name", str(val))
                else:
