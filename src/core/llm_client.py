...
async def chat_with_tools_async(messages: list[dict], tools: list[dict] | None = None,
                                temperature: float = 0.3, task: str = "general") -> LLMResponse:
...
def chat_with_tools_stream(messages: list[dict], tools: list[dict] | None = None,
                           temperature: float = 0.3, task: str = "general") -> Generator[StreamEvent, None, None]:
    provider_id, model = _resolve(task)
    client = _get_client_for_provider(provider_id)
    kwargs = {
        "model": model,
    ...
    for chunk in stream:
            ...
            for tc_delta in delta.tool_calls:
                ...
                if tc_delta.function:
                    ...
                    if tc_delta.function.arguments:
                        ...
                        yield StreamEvent(type="tool-input-delta", data={
                            "index": idx,
        ...
        if finish_reason:
            ...
            for idx, tc in sorted(current_tool_calls.items()):
                yield StreamEvent(type="tool-call-end", data={
                    "index": idx,
                    "id": tc.id,
                    "name": tc.name,
...
async def chat_with_tools_stream_async(messages: list[dict], tools: list[dict] | None = None,
    ...
    for attempt in range(max_stream_retries + 1):
        ...
        if should_retry:
            ...
            logger.warning(f"[{err_type}错误/流式] 重试 {attempt + 1}/{max_stream_retries}: "
    ...
    yield StreamEvent(type="error", data={
        "error": f"流式调用重试 {max_stream_retries} 次后仍失败: {last_error}"})
