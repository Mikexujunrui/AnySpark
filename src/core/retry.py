...
def calculate_delay(attempt: int, error: Exception | None = None) -> float:
    if isinstance(error, APIStatusError) and hasattr(error.response, "headers"):
...
async def with_retry(fn: Callable[..., Any], *args, max_retries: int = MAX_RETRIES, **kwargs) -> Any:
    ...
    if last_error is not None:
        raise last_error
    raise RuntimeError("with_retry exhausted retries but last_error is None")


async def with_retry_stream(fn: Callable[..., AsyncGenerator], *args, max_retries: int = 3, **kwargs) -> AsyncGenerator:
    last_error = None
    for attempt in range(max_retries + 1):
        try:
            ...
            error_type = "连接" if is_connection_error(e) else "服务"
            logger.warning(
                f"[{error_type}错误/流式] 重试 {attempt + 1}/{max_retries}: {e!s:.80s} → {delay:.1f}s 后重试"
            )
            await asyncio.sleep(delay)

    if last_error is not None:
        raise last_error
    raise RuntimeError("with_retry_stream exhausted retries but last_error is None")
