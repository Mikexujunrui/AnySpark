...
RELATIONSHIP_TYPES = [
    ...
    # ── Foreshadow Network v2 ──
    "HINTS_AT",       # Foreshadow → Event (暗示的结局)
    "PAID_OFF_BY",    # Foreshadow → Payoff (被回收)
    "AMPLIFIES",      # Foreshadow → Foreshadow (伏笔叠加)
    "RESOLVES_IN",    # Foreshadow → Chapter (应在何处回收)
    "CONTRASTS",      # Foreshadow → Foreshadow (对比关系)
...
RELATIONSHIP_DIRECTION: dict[str, str] = {
    ...
    # ── Foreshadow Network v2 ──
    "HINTS_AT": "unidirectional",
    "PAID_OFF_BY": "unidirectional",
    "AMPLIFIES": "unidirectional",
    "RESOLVES_IN": "unidirectional",
    "CONTRASTS": "unidirectional",
