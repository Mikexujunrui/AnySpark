...
_POSITIVE_WORDS: frozenset[str] = frozenset({
    "高兴", "快乐", "开心", "兴奋", "喜悦", "幸福", "欢喜", "欣慰", "激动",
    "感激", "感动", "温暖", "希望", "美好", "喜欢", "爱", "期待", "安心",
    "满意", "骄傲", "自信", "勇气", "光明", "胜利", "成功", "笑", "笑意",
})

_NEGATIVE_WORDS: frozenset[str] = frozenset({
    "悲伤", "痛苦", "愤怒", "恐惧", "绝望", "孤独", "寂寞", "寒冷", "黑暗",
    "死", "杀", "血", "泪", "哭", "恨", "怒", "惊", "恐", "慌", "乱",
    "失败", "失去", "离别", "背叛", "欺骗", "危险", "威胁", "压迫", "窒息",
})

# Scene-transition cue words (time / location shifts)
_SCENE_CUES: tuple[str, ...] = (
    "第二天", "次日", "三天后", "一周后", "一个月后", "数日后", "几天后",
    "此时", "与此同时", "另一边", "另一处", "回到", "来到", "走进", "走出",
    "离开", "抵达", "出发", "入夜", "清晨", "黄昏", "夜晚", "午后",
)
...
def get_chapter_pacing(book_id: str, chapter_id: str) -> ChapterPacing | None:
    ...
    try:
        ch_index = next(
            i + 1 for i, c in enumerate(regular) if c.get("id") == ch.get("id")
        )
