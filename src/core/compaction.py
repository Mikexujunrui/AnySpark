...
def _get_compaction_system() -> str:
    """Load the compaction system prompt from the template file."""
    return load_prompt("compaction_system")
...
def estimate_context_usage(messages: list[dict], model: str | None = None) -> tuple[int, int]:
...
def compact_messages(messages: list[dict]) -> list[dict]:
    ...
    while cut_point > 0:
        if conversation[cut_point].get("role") == "tool":
            cut_point -= 1
            continue
        if conversation[cut_point - 1].get("role") == "assistant" and conversation[cut_point - 1].get("tool_calls"):
            cut_point -= 1
            continue
        break
    ...
    if cut_point <= 0:
        ...
        while mid > 0 and mid < len(conversation):
            if conversation[mid].get("role") == "tool":
                mid -= 1
                continue
            if mid > 0 and conversation[mid - 1].get("role") == "assistant" and conversation[mid - 1].get("tool_calls"):
                mid -= 1
                continue
            break
        if mid <= 0:
...
def _summarize_conversation(messages: list[dict]) -> str:
    ...
    for m in messages:
        ...
        if role == "tool":
            ...
            if tool_calls:
                tools_used = ", ".join(
                    tc.get("function", {}).get("name", tc.get("name", "?")) if isinstance(tc, dict) else tc.name
...
def _aggressive_compact(messages: list[dict]) -> list[dict]:
    ...
    has_tc = any(m.get("role") == "assistant" and m.get("tool_calls") for m in result)
    if not has_tc:
