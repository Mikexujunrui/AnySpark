...
from .tools import registry, truncate_tool_output, validate_tool_input
...
class AgentConfig:
    ...
    should_stop: Callable | None = None  # (round, metrics) → bool
...
async def _loop_inner(
    ...
    try:
        while not state.exhausted:
            ...
            # and the LLM context balloons → slow responses → UI freeze.
            # Preview limit is 2000 tokens (vs old 800) — enough context
            # for the LLM to remember what happened without the bloat.
            ...
            if not response.tool_calls:
                ...
                if final_text and not _is_trivial_done(final_text):
                    ...
                    if tool_summary:
                        ...
                        done_msg = f"{final_text}（本轮共执行 {state.metrics.tool_calls} 次工具调用、{state.round} 轮）"
                else:
                    done_msg = (
            ...
            if agent_config.should_stop and agent_config.should_stop(state.round, state.metrics):
                state.metrics.finish_reason = "hook_stop"
                yield LoopEvent(
                    type="done",
                    data={
                        "message": "Agent 已由钩子终止。",
                        "rounds": state.round,
                        "parts": [p.to_dict() for p in state.parts],
                        "metrics": _loop_metrics_dict(state.metrics),
                    },
            ...
            # ── Stage 8: no-op detection — inject nudge if agent is searching without producing ──
            # When the agent spends 5+ consecutive rounds doing only search/read/list
            # operations without any write/edit/extract, it's likely stuck in a
            # "spin without producing" loop. Inject a nudge to break the cycle.
            _NOOP_READ_ONLY_TOOLS = frozenset(
                {
                    "search_knowledge",
                    "search_graph",
                    "search_reference",
            ...
            if current_tool_names and current_tool_names.issubset(_NOOP_READ_ONLY_TOOLS):
                ...
                if state.metrics.read_only_rounds >= 5:
                    _append_user_hint(
                        messages,
                        f"[系统提示] 你已经执行了 {state.metrics.read_only_rounds} 轮搜索/读取操作，"
                        "但没有任何写入或产出。请立即停止搜索，开始执行用户的实际任务。"
                        "如果需要改写章节，用 patch_chapter 或 edit_chapter。"
                        "如果需要写作，用 delegate_writing。"
                        "如果确实无法执行，请直接说明原因。",
                    )
                    state.metrics.read_only_rounds = 0  # reset after nudge
            else:
                state.metrics.read_only_rounds = 0  # reset on productive round
        ...
        if not state.metrics.finish_reason:
            ...
            try:
                ...
                done_msg = (
                    final_streamed.strip()
                    if final_streamed
                    else (f"已达循环轮次上限（{state.max_rounds} 轮），共执行 {state.metrics.tool_calls} 次工具调用。")
                )
                if not final_streamed:
                    yield LoopEvent(type="text", data={"content": done_msg})
            except Exception:
                done_msg = f"已达循环轮次上限（{state.max_rounds} 轮），共执行 {state.metrics.tool_calls} 次工具调用。"
                yield LoopEvent(type="text", data={"content": done_msg})
            yield LoopEvent(
                type="done",
                data={
...
def _prepare_initial_messages(
    ...
    # Build the user message. Dynamic context (chapter list, outline, etc.)
    # is appended here — NOT in a separate message before history — so that
    # the system prompt + history prefix stays stable across requests.
    # This is critical for LLM API prefix caching (KV cache).
    ...
    user_msg = mode_marker + _clean_msg
    if dynamic_ctx:
        user_msg = f"[当前项目上下文]\n{dynamic_ctx}\n\n{user_msg}"
...
async def _handle_tool_calls(
    ...
    # ── Prepare tool calls: validate, permissions ──
    prepared: list[dict] = []
    ...
    if state.metrics.finish_reason == "review_result":
        ...
        if not terminal_text or _is_trivial_done(terminal_text):
            ...
            if tool_summary:
                ...
                terminal_text = f"本轮共执行 {state.metrics.tool_calls} 次工具调用、{state.round} 轮。"
        yield LoopEvent(
            type="done",
            data={
...
async def _execute_tool_streaming(
    ...
    async def _run():
        ...
        try:
            ...
            if isinstance(e, (KeyboardInterrupt, SystemExit)):
                raise
            logger.error(
                "Streaming tool %s raised with args=%s: %s\n%s",
                name,
    ...
    try:
        ...
        # Generator is being closed (consumer broke early).
        # Cancel the background task and drain the queue to avoid deadlock.
        task.cancel()
        try:
            await asyncio.wait_for(task, timeout=5)
        except (asyncio.CancelledError, TimeoutError):
            pass
        # Drain any remaining items to break the consumer's await queue.get()
