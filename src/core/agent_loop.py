...
def _prepare_initial_messages(
    ...
    # Resolve genre for genre-specific system prompt
    _genre_id = ""
    if agent_config.book_id:
        try:
            from data.json_store import json_store
            _book = json_store.get_book(agent_config.book_id)
            _genre_id = _book.get("genre", "")
        except Exception:
            pass
    system_prompt = build_system_prompt(
        agent_type=agent_config.agent_type, style_name=active_style,
        genre_id=_genre_id, auto_mode_enabled=agent_config.auto_mode_enabled
    )
