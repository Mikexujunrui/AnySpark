    ...
    try:
        while not state.exhausted:
            ...
            if not response.tool_calls:
                ...
                has_incomplete, incomplete_desc = _check_incomplete_task_list(
                    agent_config.book_id, state)

                # Accept text-only as done (industry standard: no tool_calls = done)
                done_msg = final_text or "完成"
                if has_incomplete:
    ...
    for attempt in range(MAX_LLM_ERROR_RETRIES + 1):
        ...
        if attempt < MAX_LLM_ERROR_RETRIES:
            ...
            logger.warning(
                ...
                attempt + 1, MAX_LLM_ERROR_RETRIES, delay, str(error)[:120],
            )
