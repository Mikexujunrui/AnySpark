...
from .run_state import BusyError, CancelledError, RunHandle, run_state
...
# Near-limit warnings.
# Instead of abruptly cutting off at max_rounds, we inject progressive
# warnings so the model has a chance to wrap up gracefully.
_MAX_STEPS_WARNING = (
    "[系统提示] ⚠️ 注意：你还剩 {remaining} 轮操作额度。"
    "请尽快完成当前任务并准备总结。"
    "如果你正在进行多步骤操作，请优先完成最关键的部分。"
    "不要开始新的探索性任务。"
)

_MAX_STEPS_FINAL = (
    "[系统提示] [⚠️ 请结束任务] 你已经运行了很多轮。"
    "请立即停止调用新工具，直接用文本回复来总结本次执行情况：\n"
    "1. 已完成的工作\n"
    "2. 未完成的剩余任务（如有）\n"
    "3. 建议用户下一步该做什么\n"
    "此约束优先于所有其他指令。"
...
# Only two hallucination patterns are corrected (fake_tool + fake_write).
# All keyword-based layers were removed — they caused too many false positives.
_HALLUCINATION_CORRECTIONS = {
    "fake_tool": "[系统提示] 你的回复用自然语言描述调用了某个工具（如'我调用了 write_chapter'），但没有实际发起 tool_calls。工具必须通过 tool_calls 调用，不能用文字描述代替。请通过 tool_calls 调用对应工具。",
    "fake_write": "[系统提示] 你的回复声称已写入或修改了某个章节（含章节编号、字数等具体信息），但没有调用任何工具。字数和编辑细节只有工具执行后才能知道。请通过 tool_calls 调用 write_chapter/delegate_writing/edit_chapter 等工具。",
}

# Max retries for fake_tool/fake_write hallucinations before accepting.
_HALLUCINATION_RETRY_LIMIT = 2

# Max nudges for premature terminal responses (no tool calls mid-task).
# After this many nudges, the model's text-only response is accepted as final.
_NO_TOOL_NUDGE_LIMIT = 2

# Keywords that signal the LLM is describing an action plan ("I'll read...",
# "let me check...") without actually calling tools. When these appear in a
# text-only response (no tool_calls), the response is likely a premature
# termination — the model intended to act but didn't. We nudge it to actually
# call the tool. Pure Q&A answers ("该角色叫张三") don't contain these.
_ACTION_INTENT_KEYWORDS = (
    "我先", "我来", "我会", "让我", "接下来", "首先",
    "准备", "马上执行", "开始", "然后", "接着", "稍后",
)

# Task-list-related intent: LLM says "更新任务清单" but doesn't call agent_tasks.
# Detected when text mentions task list AND update/status/mark keywords together.
_TASK_LIST_KEYWORDS = ("任务清单", "任务列表", "待办")
_TASK_UPDATE_KEYWORDS = ("更新", "状态", "标记", "完成", "修改")


def _has_task_list_intent(text: str) -> bool:
    """Check if text mentions updating task list without calling the tool."""
    if not text:
        return False
    has_task = any(kw in text for kw in _TASK_LIST_KEYWORDS)
    has_update = any(kw in text for kw in _TASK_UPDATE_KEYWORDS)
    return has_task and has_update
...
class AgentConfig:
    ...
    def __post_init__(self):
        ...
        if not self.soft_round_limit:
            self.soft_round_limit = defaults.get(
                "soft_round_limit", config.agent.soft_round_limit
            )
...
async def run_agent_loop(
    ...
    try:
        ...
        yield LoopEvent(type="cancelled", data={"message": "操作已取消"})
    except Exception as e:
        logger.exception(f"Agent loop error: {e}")
        yield LoopEvent(type="error", data={"message": f"Agent 出错: {str(e)[:200]}"})
    finally:
    ...
    # hangs waiting for a termination signal.
    if not did_yield_done:
        yield LoopEvent(type="done", data={
            "message": "Agent 循环已结束（未收到明确的完成信号）。如需继续，请发起新请求。",
            "rounds": 0,
            "metrics": {},
        })
...
async def _loop_inner(
    ...
    tool_list = resolve_tools_for_agent(
        agent_config.agent_type, agent_config.mode, is_subagent=is_subagent
    )
    ...
    yield LoopEvent(type="start", data={
    ...
    try:
        while not state.exhausted:
            ...
            # ── Stage 0: progressive limit warnings ──
            # Three phases:
            #   1. Approaching soft limit (last 3 rounds before soft limit):
            #      gentle nudge, inject at rounds_remaining == 3, 2, 1.
            #   2. At soft limit (exactly): inject the "wrap up now" final nudge.
            #   3. Past soft limit (grace period before hard cap): inject the
            #      final nudge every 5 rounds. This prevents spamming the model
            #      while still providing a clear safety signal.
            if state.soft_limit_exceeded:
                if (state.round - state.last_nudge_round) >= 5:
                    _append_user_hint(messages, _MAX_STEPS_FINAL)
                    state.last_nudge_round = state.round
            elif state.rounds_remaining == 0:
                _append_user_hint(messages, _MAX_STEPS_FINAL)
                state.last_nudge_round = state.round
            elif state.is_near_limit:
                _append_user_hint(messages, _MAX_STEPS_WARNING.format(
                    remaining=state.rounds_remaining,
                ))
                state.last_nudge_round = state.round
            ...
            async for msg_type, data in _stream_llm_with_retry(
                messages, tool_list, agent_config, handle, state,
            ):
                if msg_type == "chunk":
            ...
            if fatal_error is not None:
                yield LoopEvent(type="error", data={
                    "message": f"LLM 错误（已重试{MAX_LLM_ERROR_RETRIES}次）: {str(fatal_error)[:150]}",
                })
                state.metrics.finish_reason = "llm_error"
                return
            if response is None:
                yield LoopEvent(type="error", data={"message": "LLM 未返回有效响应（已重试）"})
            ...
            if not response.tool_calls:
                ...
                # Only check for fake_tool / fake_write hallucinations.
                # These are specific, dangerous patterns that warrant a retry.
                h = detect_hallucination(final_text) if (final_text := (response.content or "").strip()) else None
                if h and h.detected and state.hallucination_retries < _HALLUCINATION_RETRY_LIMIT:
                    state.hallucination_retries += 1
                    state.metrics.record_hallucination(h.layer)
                    state.metrics.hallucination_retry_rounds += 1
                    state.rewind_round()  # Don't consume round budget for hallucination retries
                    yield LoopEvent(type="text", data={
                        "content": f"⚠️ 检测到{h.layer}类型幻觉。要求模型重新执行。\n\n[模型原回复前200字] {final_text[:200]}",
                    })
                    _append_user_hint(messages, _HALLUCINATION_CORRECTIONS.get(h.layer, ""))
                    continue  # retry → next round

                # ── Premature termination guard ──
                # If the model returned a text-only response (no tool_calls),
                # decide whether to nudge it to call tools or accept as final.
                #
                # Nudge when:
                #   1. Action-intent keywords ("我先读取...") — model described
                #      an action but didn't call the tool.
                #   2. Write mode with NO prior tool calls — model hasn't started
                #      executing yet, likely a premature termination.
                #   3. Mid-task pause with a SHORT response (≤100 chars) —
                #      model said "好的" but didn't continue.
                # Don't nudge when:
                #   - Prior tool calls + substantial text (>100 chars) — likely
                #     a legitimate completion report, not premature termination.
                #   - Pure Q&A in plan mode (no action intent, no prior tools).
                _text = streamed_text or final_text
                has_action_intent = (
                    any(kw in _text for kw in _ACTION_INTENT_KEYWORDS)
                    or _has_task_list_intent(_text)
                ) if _text else False
                is_write_mode = agent_config.mode == "write"
                has_prior_tools = state.metrics.tool_calls > 0
                is_substantial_text = len(_text) > 100 if _text else False
                needs_nudge = (
                    has_action_intent
                    or (is_write_mode and not has_prior_tools)
                    or (has_prior_tools and not is_substantial_text)
                )
                if state.no_tool_nudges < _NO_TOOL_NUDGE_LIMIT and needs_nudge:
                    state.no_tool_nudges += 1
                    # Append the assistant's text-only response so the LLM sees
                    # its own output, then inject a resume nudge.
                    messages.append({"role": "assistant", "content": _text or "（无文本输出）"})
                    # Build a context-aware nudge message
                    if _has_task_list_intent(_text or ""):
                        _nudge_msg = (
                            "[系统提示] 你说要更新任务清单，但没有通过 tool_calls 调用 agent_tasks 工具。"
                            "请立即调用 agent_tasks action=update item_index=N status=done 来更新任务状态。"
                            "不要用文字描述“我来更新任务清单”，直接通过 tool_calls 调用工具。"
                        )
                    elif state.no_tool_nudges == 1:
                        _nudge_msg = (
                            "[系统提示] 你的回复中没有 tool_calls。"
                            "工具必须通过 tool_calls 调用，不能用文字描述代替。"
                            "如果你要执行操作（读取/修改/写入/提取等），请通过 tool_calls 调用对应工具。"
                            "如果你是在回答问题，请直接给出完整答案。"
                        )
                    else:
                        _nudge_msg = (
                            "[系统提示] [⛔ 最后提醒] 你已经第二次返回没有 tool_calls 的文本。"
                            "文字描述不等于执行——必须通过 tool_calls 调用工具。"
                            "请立即通过 tool_calls 调用对应工具执行操作。"
                            "如果任务确实无法完成，请明确说明原因和阻塞点。"
                        )
                    _append_user_hint(messages, _nudge_msg)
                    yield LoopEvent(type="progress", data={"stage": f"模型未调用工具，第{state.no_tool_nudges}次提醒继续执行..."})
                    continue  # Give the model another chance to call tools

                # Accept the terminal response.
                if not streamed_text:
                    yield LoopEvent(type="text", data={"content": final_text or "完成"})
                state.metrics.finish_reason = "done"
                yield LoopEvent(type="done", data={
                    "message": final_text or "完成",
                    "rounds": state.round,
            ...
            async for ev in _handle_tool_calls(
                response, messages, agent_config, user_message, kb, handle,
                doom_detector, state, context,
            ):
                yield ev
    ...
    # ── max_rounds reached: degrade gracefully ──
    async for ev in _handle_max_rounds(agent_config, state):
        yield ev
...
def _prepare_initial_messages(
    ...
    system_prompt = build_system_prompt(agent_type=agent_config.agent_type, style_name=active_style, auto_mode_enabled=agent_config.auto_mode_enabled)
...
async def _maybe_compact(
    messages: list[dict], agent_config: AgentConfig, state: LoopState,
) -> AsyncGenerator[LoopEvent, None]:
...
async def _stream_llm_with_retry(
    ...
    """Stream one LLM attempt; on a transient error retry up to
    ``MAX_LLM_ERROR_RETRIES`` times. Yields the same tuple protocol as
    ``_stream_llm_response`` (``chunk`` / ``heartbeat`` / ``result`` /
    ``error``) plus ``progress`` notices for retries. Chunks are yielded in
    real time so the streaming UX is preserved across retries.

    Sanitization: the message-list is re-sanitized before EVERY attempt
    (including retries). This catches orphans from prior attempts inside the
    same call, plus historical corruption that may have slipped past Stage
    3.5. If all retries are exhausted and the error mentions tool_calls,
    we do one LAST-RESORT aggressive cleanup pass that strips recent
    assistant_tool_calls without responses from the tail of the history
    and tries once more — this is the "break the retry loop" move that
    prevents an unrecoverable 400 from killing the session."""
    last_error: str | None = None
    for attempt in range(MAX_LLM_ERROR_RETRIES + 1):
        # Re-sanitize on every attempt. Idempotent, so safe for repeated calls.
        _sanitize_tool_messages(messages)
        if attempt > 0:
        ...
        async for msg_type, data in _stream_llm_response(
            messages, tools, agent_config, handle, state.temperature,
        ):
            if msg_type == "error":
                error = data
                last_error = data
                ...
                # chunk / heartbeat — forward immediately
                yield (msg_type, data)
        if error is None and response is not None:
            yield ("result", (response, streamed_text))
            return

    # ── Last-resort recovery for tool_call orphan 400s ──
    # If all retries failed with the specific "tool_calls" error, the message
    # history likely has unrecoverable corruption from a previous session or
    # partial tool processing. Strip trailing assistant_tool_calls messages
    # that have no tool responses in the tail, then make one final attempt.
    if last_error is not None and _is_tool_call_orphan_error(last_error):
        logger.warning("Last-resort recovery: stripping orphaned assistant tool_calls from tail")
        if _aggressive_strip_trailing_orphans(messages):
            yield ("progress", "检测到消息历史损坏，正在尝试修复...")
            _sanitize_tool_messages(messages)
            response = None
            streamed_text = ""
            error = None
            async for msg_type, data in _stream_llm_response(
                ...
                if msg_type == "error":
                    error = data
                    # Don't save to last_error; it's already been reported.
                elif msg_type == "result":
                    response, streamed_text = data
                else:
                    yield (msg_type, data)
            if error is None and response is not None:
                yield ("result", (response, streamed_text))
                return
            logger.error("Last-resort recovery failed. Error: %s", str(error)[:200])
...
def _is_tool_call_orphan_error(error: str) -> bool:
    """Detect the specific OpenAI 400 error about orphaned tool_call_ids."""
    if not error:
        return False
    msg = str(error)
    return ("tool_call" in msg and "tool message" in msg) or \
           ("tool_calls" in msg and "must be followed" in msg)


def _aggressive_strip_trailing_orphans(messages: list[dict]) -> bool:
    """Walk backward through messages and drop trailing assistant_tool_calls
    messages that don't have any tool responses after them. Used as a last
    resort when normal sanitization failed. Returns True if messages changed."""
    if not messages:
        return False
    # Find the last assistant_with_tool_calls message
    ...
    if last_assistant_idx is None:
        return False
    # Check if there's any tool message after it that matches its call ids
    call_ids = {
    ...
    for j in range(last_assistant_idx + 1, len(messages)):
        if messages[j].get("role") == "tool" and messages[j].get("tool_call_id") in call_ids:
            has_response = True
            break
    if has_response:
        # Something else is wrong — strip anyway, up to the last 3 messages to give the
        # API a clean tail to work with.
        if len(messages) > 1:
            # Drop messages after the last assistant_with_tool_calls if they look weird
            tail_start = last_assistant_idx + 1
            # Keep any user/system messages but drop everything else after the orphan assistant
            new_tail = [m for m in messages[tail_start:] if m.get("role") in ("user", "system")
                        and "[系统提示]" not in (m.get("content", "") or "")]
            # Cap at one user message to not lose context
            new_tail = new_tail[-1:] if new_tail else []
            messages[last_assistant_idx + 1:] = new_tail
            # Now the orphan assistant is at the end with no tool response — drop it
            del messages[last_assistant_idx]
            return True
        return False
    # No tool responses after it → drop the orphan assistant
    del messages[last_assistant_idx]
    return True
...
async def _handle_tool_calls(
    ...
    # ── Drift detection (completion-claim and verify-loop kept separate) ──
    if streamed_text and len(streamed_text) > 5:
        ...
        if state.drift_limit_reached:
            yield LoopEvent(type="text-correction", data={
                "reason": f"第{state.completion_claims + state.verify_loops}次{state.drift_signal_label}，已要求停止",
            })
    ...
    # ── Drift hard stop — but allow one read-only tail call ──
    tool_name_set = {tc.name for tc in response.tool_calls}
    if state.drift_corrected and not state.allow_tail_tools(tool_name_set):
        yield LoopEvent(type="text-correction", data={
            "reason": "忽略漂移纠正继续调用工具，强制结束",
        })
        state.metrics.finish_reason = "drift_force_stop"
        yield LoopEvent(type="done", data={
            "message": "已检测到循环执行，强制停止。请检查当前结果，如需继续操作请发起新请求。",
            "rounds": state.round,
            "metrics": _loop_metrics_dict(state.metrics),
        })
        return
    if state.drift_corrected and state.allow_tail_tools(tool_name_set):
        state.consume_tail()

    # ── Prepare tool calls: validate, permissions, doom loop ──
    prepared: list[dict] = []
    async for ev in _prepare_tool_calls(
        response, messages, agent_config, handle, doom_detector, state, prepared,
    ):
        yield ev
    ...
    # Tools are being executed — reset retry/nudge counters.
    state.hallucination_retries = 0
    state.no_tool_nudges = 0  # Reset: LLM called tools, so prior nudges are resolved

    # ── KB mutation guard (sliding window, metadata-driven) ──
    from .tool_meta import READ_TOOLS
    tool_name_set = {tc.name for tc in response.tool_calls}
    _this_round_mutates_kb = any(
        getattr(registry.get(name), "mutates_kb", False) for name in tool_name_set
    )
    if _this_round_mutates_kb:
        state.record_kb_round()
        if state.kb_mutation_limit_reached:
            yield LoopEvent(type="text-correction", data={
                "reason": f"连续{state.kb_mutation_streak}轮知识库修改操作，强制停止",
            })
            _append_user_hint(messages, _build_kb_overload_correction(user_message, state))
    ...
    yield LoopEvent(type="tool-start", data={
        "tool": ", ".join(p["tc"].name for p in prepared),
        "args": (f"{len(prepared)} 个工具并行执行" if len(prepared) > 1
                  else _safe_args_preview(prepared[0]["args"])),
    })

    # Record tool-call parts for history persistence (full args, not preview).
    for p in prepared:
        state.add_part(ToolCallPart(
    ...
    for p in streaming_tasks:
        ...
        try:
            ...
            messages.append({
                ...
                "content": f"工具 {tc.name} 处理结果时出错: {str(e)[:200]}"
            })
    ...
    yield LoopEvent(type="tool-end", data={
        "tool": ", ".join(p["tc"].name for p in prepared),
    ...
    # ── Terminal tool result (e.g. review_result) — signal done and stop ──
    if state.metrics.finish_reason == "review_result":
    ...
    try:
        for tc in response.tool_calls:
            ...
            if doom_detector.record_call(tc.name, tc.arguments):
                messages.append({
                    "role": "tool", "tool_call_id": tc.id,
                    "content": f"[错误] 死循环检测：{tc.name} 连续相同调用。请换方式处理。"
                })
                processed_ids.add(tc.id)
                state.metrics.doom_loop_skips += 1
                yield LoopEvent(type="doom-loop", data={"tool": tc.name})
                continue
            ...
            if tool_def:
                ...
                if errors:
                    messages.append({"role": "tool", "tool_call_id": tc.id, "content": f"参数校验失败: {'; '.join(errors)}"})
                    processed_ids.add(tc.id)
                    continue
                args = validated_args
            ...
            if perm_action == "ask":
                ...
                q_req = question_manager.create_question(
                    [{"question": confirm_msg, "header": "权限确认",
                      "options": [{"label": "确认执行", "description": "允许"},
        ...
        logger.info("_prepare_tool_calls cancelled mid-loop, backfilling %d remaining",
        ...
        while remaining > 0:
            ...
            try:
                answers = await asyncio.wait_for(
                    question_manager.wait_for_answer(question_id), timeout=chunk
                )
...
async def _process_tool_result(
    ...
    if isinstance(result, dict) and result.get("type") == "plot_cards":
        ...
        q_req = question_manager.create_question(
            [{"question": "选择一个剧情方向", "header": "剧情走向",
              "options": [{"label": c.get("title", ""), "description": c.get("description", "")} for c in cards],
              "card_type": "plot_cards", "cards": cards,
              "context_summary": result.get("context_summary", ""), "custom": True}],
            agent_config.book_id)
        yield LoopEvent(type="plot_cards", data={
            "id": q_req.id, "context_summary": result.get("context_summary", ""),
            "cards": cards, "instruction": result.get("instruction", "")})
        try:
        ...
        ch_list = "、".join(
            f"第{c['index']}章{c.get('title','')}" for c in chapters[:5]
        )
        confirm_msg = (
        ...
        q_req = question_manager.create_question(
            [{"question": confirm_msg, "header": "启动 Autopilot",
              "options": [
        ...
        if result.get("saved"):
            yield LoopEvent(type="writing_end", data={
        ...
        if not result.get("error"):
            yield LoopEvent(type="patch_result", data={
                "chapter_id": result.get("chapter_id", ""),
                "chapter_title": result.get("chapter_title", ""),
        ...
        if qs:
            ...
            try:
                answers = await asyncio.wait_for(
                    question_manager.wait_for_answer(q_req.id), timeout=300)
            except TimeoutError:
                answers = [["用户超时未回复"]]
            except Exception:
                answers = [["用户拒绝了提问"]]
...
# ─────────────────────────────────────────────────────────────────────────────
# Stage: graceful max_rounds handling
# ─────────────────────────────────────────────────────────────────────────────

async def _handle_max_rounds(
    agent_config: AgentConfig, state: LoopState,
) -> AsyncGenerator[LoopEvent, None]:
    """Fallback when the model ignored the max-steps prompt and kept calling
    tools. Instead of silently stopping, ask for a one-line status.

    This is the "safety net" — the preferred path is that the model sees the
    ``_MAX_STEPS_FINAL`` prompt injected in the last round and voluntarily
    outputs a text-only summary, which the loop's normal terminal-branch
    handling picks up. This function only runs if the model somehow burned
    through the last round with tool calls."""
    summary = ""
    try:
        summary = chat(
            "请用一两句话汇报本次任务的完成情况：已完成什么、未完成什么、建议下一步。不要再调用任何工具。",
            system="你是写作助手。用简短的要点汇报已完成的操作和未完成的剩余工作，不要调用工具。",
            temperature=0.2,
            task=agent_config.task_label,
        )
    except Exception as e:
        logger.warning(f"_handle_max_rounds chat call failed: {e}")
        summary = ""
    state.metrics.finish_reason = "max_rounds"
    text = summary.strip() if summary and summary.strip() else "已达到最大执行轮次，自动停止。如需继续，请发起新请求。"
    yield LoopEvent(type="text", data={"content": text})
    yield LoopEvent(type="done", data={"message": text, "rounds": state.round, "metrics": _loop_metrics_dict(state.metrics)})
...
def _collect_result_parts(tc: ToolCall, result, state: LoopState, messages: list[dict]) -> None:
    ...
    for m in reversed(messages):
        if m.get("role") == "tool" and m.get("tool_call_id") == tc.id:
            ...
            break
    state.add_part(ToolResultPart(
        tool_call_id=tc.id, result=result_str,
    ...
    if isinstance(result, dict):
        ...
        if rtype == "writing_result" and result.get("saved"):
            state.add_part(ChapterDiffPart(
                chapter_id=result.get("chapter_id", ""),
                chapter_title=result.get("chapter_title", ""),
                operation="created",
                word_count=result.get("word_count", 0),
...
def _build_sitrep_sync(agent_config: "AgentConfig") -> str:
    ...
    try:
        ...
        total_words = sum(
            len(json_store._chapter_view(c).get("content", "")) for c in chapters
        ) if chapters else 0
...
async def _execute_tool_streaming(name: str, args: dict, agent_config: AgentConfig, user_message: str, kb, context: AgentContext | None = None):
    ...
    async def _run():
        result = await execute_tool_streaming(
            loop, name, args, kb, agent_config.book_id,
            user_message, agent_config.session_id, queue,
            context=context,
        )
...
async def _execute_tool(name: str, args: dict, agent_config: AgentConfig, user_message: str, kb, context: AgentContext | None = None) -> str | dict:
    from tools.executor import execute_tool

    loop = asyncio.get_running_loop()
    return await execute_tool(
        loop, name, args, kb, agent_config.book_id,
        user_message, agent_config.session_id, confirmed=True,
        context=context,
    )
