...
class ContextManager:
    ...
    def build_scoped_context(self, scope) -> str:
        ...
        if scope.structure_report:
            try:
                ...
                struct_rpt = StructureReport(**scope.structure_report)
                fragment = struct_rpt.to_prompt_fragment()
                if fragment:
                    structural_sections.append(f"\n## 原著结构参考\n{fragment}")
            except Exception:
                pass  # best-effort injection
        ...
        if scope.sentence_rhythm:
            try:
                ...
                rhythm = SentenceRhythm(**scope.sentence_rhythm)
                fragment = rhythm.to_prompt_fragment()
                if fragment:
                    structural_sections.append(fragment)
            except Exception:
                pass
        ...
        for _, label, exposures in type_groups:
            ...
            for exp in exposures:
                ...
                if exp.level == ExposureLevel.FULL:
                    phase_override, phase_label = self._resolve_phase_for_entity(entity)
            ...
            try:
                ...
                if shared_conns:
                    ...
                    for conn in shared_conns:
                        ...
                        if from_e and to_e:
                            rel_lines.append(f"- {from_e.name} --[{conn['type']}]--> {to_e.name}")
                    rel_block = "\n".join(rel_lines)
            ...
            try:
                ...
                for exp in scope.characters[:5]:
                    if exp.level.value == "full":
                        voice_fp = get_character_voice(self.project_id, exp.entity_name)
                        if voice_fp and voice_fp.dialogue_count >= 3:
                            prompt = build_voice_prompt(voice_fp)
                            if prompt:
    ...
    def _calculate_entity_priority(self, entity, scope_entity_ids: set[str] | None = None) -> int:
        """Assign a priority tier (1-3) to an entity for context injection.

        Tier 1 (must-inject): entities explicitly in the writing scope, or
        entities linked to scheduled/planned foreshadows.
        Tier 2 (inject-if-budget): direct relations of Tier-1 entities, or
        characters with active phase snapshots.
        Tier 3 (inject-if-remaining): background worldbuilding, locations,
        historical events.

        Returns 1 (highest priority), 2, or 3 (lowest).
        """
        # Tier 1: scope entities
        if scope_entity_ids and entity.id in scope_entity_ids:
            return 1

        # Tier 1: character type entities are always high priority
        if entity.type == EntityType.CHARACTER:
            # Check if entity has active role data
            if entity.data.get("role") or entity.data.get("personality"):
                return 1
            return 2

        # Tier 1: entities with active phase snapshots
        try:
            phase = self.store.get_current_phase(entity.id)
        ...
        # Tier 2: location and item entities
        if entity.type in (EntityType.LOCATION, EntityType.ITEM, EntityType.ORGANIZATION):
            return 2

        # Tier 3: concept, event, skill, race — worldbuilding background
        return 3

    def _format_entity_summary(self, entity, data_override: dict | None = None, phase_label: str = "") -> str:
    ...
    def build_writing_context(
        ...
        if pov_character_id:
            try:
                ...
                if chapter_number is not None:
                    knowledge = self.store.get_character_knowledge(pov_character_id, chapter_number)
                ...
                sections.append(
                    f"\n## 视角限制\n当前以角色视角过滤上下文，仅注入该角色可感知的 {len(all_entities)} 个实体。"
                )
            except Exception:
        ...
        if relevant_entity_names:
            ...
            if len(resident_added) >= 2:
                try:
                    shared_conns = self.store.find_share_connections([e.id for e in resident_added])
                    if shared_conns:
                        entity_by_id = {e.id: e for e in all_entities}
                        scene_lines = ["\n## 同场实体关系矩阵"]
                        for conn in shared_conns:
                            ...
                            to_e = entity_by_id.get(conn["to"])
                            if from_e and to_e:
                                scene_lines.append(f"- {from_e.name} ↔[{conn['type']}]↔ {to_e.name}")
                        scene_block = "\n".join(scene_lines)
        ...
        if mode == "write" and instruction:
            ...
            remaining = [
                e
                for e in remaining
                if e.name.lower() in instr_lower or any(a.lower() in instr_lower for a in e.aliases)
            ]

        # ── Priority-based sorting: higher-priority entities listed first ──
        # Uses _calculate_entity_priority() which considers entity type,
        # phase data, and scope membership.
        try:
            remaining.sort(key=lambda e: self._calculate_entity_priority(e))
        except Exception:
            pass  # best-effort; falls back to type-order below
    ...
    def _format_entity_detail(
        ...
        direct_ids = (
            {r.to_entity if r.from_entity == entity.id else r.from_entity for r in relations} if relations else set()
        )
        try:
            neighbors = store.get_neighbors(entity.id, depth=2)
            indirect = [n for n in neighbors if n["entity"].id != entity.id and n["entity"].id not in direct_ids]
            if indirect:
