...
def import_spark(book_id: str, archive_path: str) -> dict:
    ...
    stats = {"entities": 0, "relations": 0, "foreshadows": 0,
             "timeline_events": 0, "chapters": 0, "errors": []}
    ...
    try:
        ...
        for e in entities:
            conn.execute(
                ...
                (e.id, e.type, e.name, json.dumps(e.aliases, ensure_ascii=False),
                 json.dumps(e.data, ensure_ascii=False), book_id),
            )
        ...
        for e in entities:
            for r in store.list_relations(e.id):
                conn.execute(
                    ...
                    (r.id, r.from_entity, r.to_entity, r.type.value,
                     json.dumps(r.data, ensure_ascii=False), book_id),
                )
    except Exception as exc:
        ...
        for status in ("open", "planned", "due", "scheduled", "resolved", "cross_volume", "dangling"):
            for f in store.list_foreshadows(status=status):
                conn.execute(
                    ...
                    (f.id, f.text, f.hint, f.expected_resolution,
                     1 if f.resolved else 0, f.resolution_text,
                     json.dumps(f.related_entities, ensure_ascii=False),
                     json.dumps(f.related_events, ensure_ascii=False),
                     f.source, f.status, f.plant_chapter, f.resolve_chapter,
                     f.volume_ref, f.planned_resolve_arc,
                     f.scheduled_chapter, f.confidence,
...
def _import_graph_from_sqlite(book_id: str, db_path: Path) -> dict:
    ...
    # Entities
    rows = conn.execute("SELECT id, type, name, aliases, data FROM entities WHERE book_id=?",
                        (book_id,)).fetchall()
    for row in rows:
        try:
            entity = Entity(
    ...
    # Relations
    rows = conn.execute("SELECT id, from_entity, to_entity, type, data FROM relations WHERE book_id=?",
                        (book_id,)).fetchall()
    for row in rows:
        try:
        ...
        try:
            relation = Relation(
                id=row[0], from_entity=row[1], to_entity=row[2],
                type=rtype,
                data=json.loads(row[4]) if row[4] else {},
            )
    ...
    for row in rows:
        try:
            fs = Foreshadow(
                id=row[0], text=row[1], hint=row[2],
                expected_resolution=row[3] or "",
                resolved=bool(row[4]), resolution_text=row[5] or "",
                related_entities=json.loads(row[6]) if row[6] else [],
                related_events=json.loads(row[7]) if row[7] else [],
                source=row[8] or "extracted", status=row[9] or "open",
                plant_chapter=row[10] or "", resolve_chapter=row[11] or "",
                volume_ref=row[12] or "", planned_resolve_arc=row[13] or "",
                scheduled_chapter=row[14] or "", confidence=row[15] or "high",
