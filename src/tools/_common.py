...
# 输出格式（严格JSON）
{
  ...
  "foreshadows": [
    {"text": "伏笔原文", "hint": "暗示内容", "expected_resolution": "可能的揭晓方式"}
  ]
}

关系类型限定: ally, antagonist, family, romantic, master_of, owns, located_at, belongs_to, causes, participates_in, knows
如果本章没有新信息可提取，输出 {"new_entities":[],"updates":[],"relations":[],"foreshadows":[]}"""
...
def _local_skip_check(content: str, known_names: set) -> bool:
    names_in_text = set(re.findall(
        r'[\u4e00-\u9fff]{2,4}(?=[说道想笑叹看问答喊叫])', content[:5000]))
    new_names = names_in_text - known_names
    setting_keywords = {"世界", "魔法", "功法", "境界", "种族", "国度", "势力", "宗门",
                        "秘境", "血脉", "神器", "阵法", "丹药", "灵石", "大陆", "帝国"}
...
def _parse_progressive_result(response: str) -> dict:
    ...
    try:
        ...
        return {"new_entities": [], "updates": [],
                "relations": [], "foreshadows": []}


def _apply_progressive_result_batch(
        result: dict, kb, book_id: str) -> tuple[int, int, int, int]:
    import uuid
    ...
    for r in result.get("relations", []):
        ...
        to_name = r.get("to", "")
        from_id = name_to_id.get(from_name) or name_to_id.get(
            from_name.lower()) or from_name
        to_id = name_to_id.get(to_name) or name_to_id.get(
            to_name.lower()) or to_name
    ...
    for f in result.get("foreshadows", []):
        foreshadows_to_add.append(Foreshadow(
            id=str(uuid.uuid4())[:8],
            text=f.get("text", ""),
            hint=f.get("hint", ""),
    ...
    return new_count, update_count, len(
        relations_to_add), len(foreshadows_to_add)
