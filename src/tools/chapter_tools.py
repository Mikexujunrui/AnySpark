...
请生成全书摘要（JSON格式）。"""
    ...
    result_str = await loop.run_in_executor(ai_executor, llm_chat, prompt, system, 0.3, "analysis")
    ...
    char_list = ", ".join(f"{c.get('name', '?')}({c.get('role', '?')})" for c in summary.get("characters", [])[:5])
    return (
        f"全书摘要已生成并保存。\n"
        f"  书名: {book_title}\n"
...
4. 如果指令不适用于本章（如章节中没有相关内容），原样输出不修改"""
            ...
            async with semaphore:
                    ...
                    try:
                        pre_result = await asyncio.wait_for(
                            loop.run_in_executor(
                                ai_executor, llm_chat, precheck_prompt, precheck_system, 0.0, "extraction"
                            ),
                            timeout=15,
                        )
                ...
                try:
                    new_content = await asyncio.wait_for(
                        loop.run_in_executor(ai_executor, _call_edit_llm, llm_chat, prompt, edit_system),
    ...
    if precheck and precheck_count > 0:
        ...
        summary.append(
            f"  🔍 预检: {precheck_hit}/{precheck_count} 章命中（{hit_rate:.0f}%），跳过 {precheck_count - precheck_hit} 章"
        )
    summary += ["", "逐章结果:"]
