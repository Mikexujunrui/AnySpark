...
def _auto_register_tools() -> None:
    ...
    for _name, _handler in _DISPATCH.items():
        ...
        for _pname, _p in _sig.parameters.items():
            ...
            _ts_type = {str: "string", int: "integer", float: "number", bool: "boolean", dict: "object"}.get(
                _py_type, "string"
            )
            _params[_pname] = {
                "type": _ts_type,
        ...
        logger.info(
            "Auto-registered tool: %s (from %s.%s)",
            _name,
...
async def execute_tool_streaming(
    loop, name: str, args: dict, kb, book_id: str, msg: str, session_id: str, queue, context: AgentContext | None = None
) -> str | dict:
    # ── Plan-mode runtime guard: reject tools that mutate KB or touch chapters ──
    if context is not None and context.is_plan_mode:
        meta = TOOL_META.get(name, {})
        if meta.get("mutates_kb") or meta.get("touches_chapter"):
            logger.warning("Plan-mode guard blocked write tool %r", name)
            return f"❌ 当前是 Plan 模式，禁止调用写入工具「{name}」。如需写作请切换到 Write 模式。"
    try:
        _build_dispatch()
        ...
        if isinstance(e, (KeyboardInterrupt, SystemExit)):
            raise
        logger.error(
            "Streaming tool %s failed with args=%s: %s\n%s",
            name,
...
async def execute_tool(
    ...
    try:
        ...
        if handler is not None:
            result = await _call_handler(handler, loop, args, kb, book_id, msg, session_id, context)
            bus.emit_sync(
                Event(type=EventType.TOOL_EXECUTED, data={"tool": name, "book_id": book_id}, source="executor")
            )
            return result
        ...
        if name in ("manage_volumes", "generate_volume_outlines", "list_volumes"):
            result = _handle_volume(
                ...
                }.get(args.get("action", "list"), "list_volumes"),
                args,
                book_id,
            )
            return result
        ...
        if name in ("transform_book", "find_replace_book", "summarize_book"):
            ...
            _lazy = {"find_replace_book": find_replace_book, "summarize_book": summarize_book}
            return await _lazy[name](loop, args, book_id)

        return f"工具 {name} 未注册 (params: {json.dumps(args, ensure_ascii=False)[:100]})"

    except ToolExecutionError as e:
        bus.emit_sync(Event(type=EventType.TOOL_FAILED, data={"tool": name, "error": str(e)[:100]}, source="executor"))
...
async def _run_sub_agent(args: dict, book_id: str, session_id: str, context: AgentContext | None = None) -> str:
    ...
    if context is not None and context.is_plan_mode and agent_type not in READONLY_SUBAGENT_TYPES:
        ...
        logger.warning("Plan-mode guard blocked spawn of write-type sub-agent %r", agent_type)
        return (
    ...
    if fts_results:
        ...
        for r in fts_results:
            ...
            if e:
                ...
                # Show key attributes
                key_attrs = []
                for attr in ("role", "personality", "abilities", "location", "description", "function", "effect"):
                    val = e.data.get(attr, "")
                    if val:
        ...
        if len(matched_entities) >= 2:
            try:
                shared = kb.find_share_connections([e.id for e in matched_entities])
                if shared:
                    rel_lines = ["\n  🔗 匹配实体间的关系:"]
        ...
        matching = [e.name for e in entities if query.lower() in e.name.lower() or query in str(e.data).lower()]
        lines.append(", ".join(matching[:10]) or "无匹配")
    return "\n".join(lines)


async def _extract_knowledge(loop, args: dict, kb, book_id: str, msg: str) -> str:
...
async def _get_voice_profile_tool(loop, book_id: str) -> str:
    ...
    for fp in fingerprints:
        if fp.dialogue_count > 0:
            lines.append(
                f"- **{fp.character_name}**：{fp.emotional_tendency}，平均句长{fp.avg_sentence_length:.0f}字，{fp.dialogue_count}句对话"
            )
        else:
