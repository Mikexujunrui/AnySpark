...
# ── read_chapter de-duplication cache ──
# Prevents the agent from reading the same chapter multiple times per session,
# which balloons context with duplicate full-text content.
# Key: (book_id, chapter_id), Value: (content_preview_chars, word_count, title)
# Cleared on server restart; each session is typically short-lived enough
# that this is sufficient.
_read_cache: dict[tuple[str, str], str] = {}  # (book_id, chapter_id) → preview_text
_CACHE_PREVIEW_CHARS = 800  # characters to return on re-read
_CACHE_HINT = (
    "\n\n[ℹ️ 你已在当前会话中读取过本章。"
    "不需要再次读取全文，请基于已有内容继续操作。"
    "如需查看特定段落，使用 offset 和 limit 参数。]"
)
...
def _list_chapters(book_id: str) -> str:
    ...
    for c in chapters:
        ...
        if is_extra:
            extra_idx += 1
            lines.append(f"- #E{extra_idx} [番外] {view.get('title', '?')[:30]} | {chars}字 | v{vc}")
        else:
            regular_idx += 1
            lines.append(f"- #{regular_idx} {view.get('title', '?')[:30]} | {chars}字 | v{vc}")
...
def _read_chapter(args: dict, book_id: str) -> str:
    ...
    # ── De-duplication: if the agent already read this chapter, return a preview ──
    cache_key = (target_book_id, cid)
    if offset == 0 and limit is None and cache_key in _read_cache:
    ...
    try:
        ...
        result = content[offset:] if offset else content

        # ── Cache a preview for de-duplication on subsequent reads ──
        if offset == 0 and limit is None:
            preview = result[:_CACHE_PREVIEW_CHARS]
            if len(result) > _CACHE_PREVIEW_CHARS:
                preview += "..."
            preview += _CACHE_HINT
            _read_cache[cache_key] = preview  # cache preview, not full content
...
def _edit_chapter(args: dict, book_id: str) -> dict:
    ...
    try:
        result = json_store.edit_chapter(book_id, chapter_id, content, title=title, message=message)
        return {
            "type": "writing_result",
...
def _patch_chapter(args: dict, book_id: str) -> dict:
    ...
    try:
        ...
        for i, op in enumerate(patches):
            ...
            op_result = {"index": i + 1, "op": op_type, "success": True}
            # 根据操作类型提取关键信息
        ...
        lines = [
            f"已应用 {patched}/{len(patches)} 个 patch: {result.get('title', '')} "
...
def _reorder_chapters(args: dict, book_id: str) -> dict:
    order = args.get("order", [])
    if not order or not isinstance(order, list):
        return {"type": "reorder_result", "error": "order 必须是章节 ID 列表"}
    try:
        result = json_store.reorder_chapters(book_id, order)
        return {
            "type": "reorder_result",
            "text": f"已重排 {result['count']} 个章节",
            "count": result["count"],
        }
    except Exception as e:
        return {"type": "reorder_result", "error": f"章节重排失败: {str(e)[:200]}"}
...
def _chapter_history(args: dict, book_id: str) -> str:
    ...
    try:
        ...
        for v in history:
            ...
            lines.append(
                f"- [{v['id'][:12]}] {v['timestamp'][:16]} | {v['message'][:30]} | {v['word_count']}字{marker}"
            )
        return "\n".join(lines)
    except Exception as e:
...
def _revert_chapter(args: dict, book_id: str) -> str:
    ...
    try:
        ...
        return (
            f"已回退到版本 {result.get('current_version', '')[:12]}: "
...
async def _diff_chapters(loop, args: dict, book_id: str) -> str:
    ...
    try:
        ...
        if version_b:
            ...
            vb = {"content": ch.get("content", ""), "id": ch.get("current_version", "now")}
        ...
        diff = list(
            difflib.unified_diff(
                ...
                tofile=f"{vb['id'][:12]} ({vb.get('message', '')})"
                if isinstance(vb, dict) and "message" in vb
                else vb.get("id", "current")[:12],
                lineterm="",
                n=2,
            )
        )
