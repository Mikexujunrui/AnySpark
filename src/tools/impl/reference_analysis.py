...
def _format_structure_output(data: dict) -> str:
    ...
    if para.get("avg_per_chapter"):
        lines.append(f"段落: 平均每章 {para['avg_per_chapter']:.0f} 段，每段约 {para['avg_length']:.0f} 字")

    sent = data.get("sentence_stats", {})
    if sent.get("avg_per_chapter"):
        lines.append(f"句子: 平均每章 {sent['avg_per_chapter']:.0f} 句，每句约 {sent['avg_length']:.0f} 字")
...
def _format_style_output(data: dict) -> str:
    ...
    if para.get("mean"):
        lines.append(
            f"段落长度: 均值{para['mean']:.0f}字, 中位数{para.get('median', 0):.0f}字, 标准差{para.get('std', 0):.0f}"
        )
...
async def _analyze_structure_tool(loop, args: dict, book_id: str) -> str:
    ...
    report = await loop.run_in_executor(_ai_executor, analyze_structure, ref_book_id)
...
DEEP_ANALYSIS_TYPES = {
    "sentence_rhythm": "句式韵律分析",
    "rhetoric_density": "修辞手法密度分析",
    "prophecy_signature": "谶语/预叙特征分析",
    "narrative_pov": "叙事视角分析",
}


def _format_deep_analysis_output(data: dict, analysis_type: str) -> str:
    """Format a deep analysis dict as human-readable output."""
    name = DEEP_ANALYSIS_TYPES.get(analysis_type, analysis_type)
    lines = [f"## {name}"]
    for key, value in data.items():
        if key in ("book_id", "chapter_count"):
            continue
        if isinstance(value, float):
            ...
            top = sorted(value.items(), key=lambda x: -x[1])[:3]
            lines.append(f"  {key}: {', '.join(f'{k}={v}' for k, v in top)}")
        else:
            lines.append(f"  {key}: {value}")
    lines.append("\n写作时将自动注入以上分析结果作为风格约束。")
    return "\n".join(lines)


async def _analyze_deep_style_tool(loop, args: dict, book_id: str) -> str:
    """Tool handler for deep style analysis (sentence_rhythm, rhetoric_density, etc.)."""
    ...
    analysis_type = args.get("analysis_type", "sentence_rhythm")
    if analysis_type not in DEEP_ANALYSIS_TYPES:
        return f"错误: 不支持的分析类型 '{analysis_type}'。支持: {', '.join(DEEP_ANALYSIS_TYPES.keys())}"

    # Check cache
    cached = load_analysis(analysis_type, ref_book_id)
    if cached:
        return f"（已缓存）{_format_deep_analysis_output(cached, analysis_type)}"
    ...
    if not analyzer:
        return f"错误: 找不到分析器 {analysis_type}"

    result = await loop.run_in_executor(_ai_executor, analyzer, ref_book_id)
    if not result.chapter_count:
        return f"错误: 参考书 {ref_book_id} 没有章节内容。"

    return _format_deep_analysis_output(result.to_dict(), analysis_type)


async def _analyze_emotional_curve_tool(loop, args: dict, book_id: str) -> str:
    """Tool handler for emotional curve analysis."""
    ...
    cached = load_emotional_curve(ref_book_id)
    if cached:
        return f"（已缓存）{_format_deep_analysis_output(cached, 'emotional_curve')}"

    result = await loop.run_in_executor(_ai_executor, analyze_emotional_curve, ref_book_id)
    if not result.chapter_count:
        return f"错误: 参考书 {ref_book_id} 没有章节内容。"

    return _format_deep_analysis_output(result.to_dict(), "emotional_curve")
...
async def _quantify_style_tool(loop, args: dict, book_id: str) -> str:
    ...
    # Execute analysis (pure Python, runs in thread pool)
    fingerprint = await loop.run_in_executor(_ai_executor, quantify_style, ref_book_id)
