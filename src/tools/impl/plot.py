    ...
    if outline.get("chapters"):
        ...
        for i, c in enumerate(outline["chapters"]):
            if c and c.get("synopsis"):
                ch_lines.append(
                    f"第{i + 1}章 {c.get('title', '')}: {c['synopsis'][:60]}")
        if ch_lines:
...
原则：每卷8-25章，有独立叙事弧；storyLine描述本卷而非全书；已有分卷跳过。只输出JSON。"""
    ...
    try:
        response = asyncio.get_event_loop().run_until_complete(
            asyncio.to_thread(chat, planner_prompt,
                system="你是小说结构规划师。输出 JSON。", temperature=0.2, task="general")
        )
    except RuntimeError:
...
输出JSON: {"avg_sentence_length":"","description_density":"high/medium/low","dialogue_ratio":"","pace":"fast/medium/slow","tone":"","vocabulary_features":[],"distinctive_patterns":[],"narrative_pov":"","paragraph_structure":""}"""
    result = await loop.run_in_executor(
        _ai_executor, llm_chat,
        f"样本文本:\n{sample[:config.storage.max_style_sample_chars]}", system, 0.2, "extraction"
    )
    style_id = str(int(datetime.now().timestamp() * 1000))
    entity = Entity(id=style_id, type=EntityType.CONCEPT, name=f"StyleProfile_{book_id[:8]}",
                    data={"type": "style_profile", "analysis": result[:3000]})
...
async def _reconstruct_chapter(loop, args: dict, kb, book_id: str) -> str:
    ...
    summary = kb.get_knowledge_summary()[
        :config.storage.max_knowledge_summary_chars]
...
def _read_document(args: dict, session_id: str, book_id: str) -> str:
    ...
    if not doc_id:
        ...
        doc_list = ", ".join(
            f"ID={d['id']}: {d['filename']} ({d['chars']}字)" for d in docs
        )
