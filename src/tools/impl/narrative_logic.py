...
async def _search_graph(loop, args: dict, kb, book_id: str, msg: str = "") -> str:
    ...
    try:
        result = await loop.run_in_executor(
            _ai_executor, graph_search, kb, question
        )
    except Exception as e:
        return f"图谱搜索失败: {e}"
...
async def _get_graph_insights(loop, args: dict, kb, book_id: str, msg: str = "") -> str:
    ...
    try:
        insights = await loop.run_in_executor(
            _ai_executor, kb.get_graph_insights
        )
    except Exception as e:
        return f"图谱洞察获取失败: {e}"
    ...
    if scores:
        ...
        if weak:
            lines.append(f"\n### 设定薄弱 ({len(weak)}个)")
            for s in weak[:5]:
                lines.append(f"  - {s.get('entity_name', '?')} ({s.get('entity_type', '')}): {s.get('recommendation', '')}")
...
async def _verify_chapter(loop, args: dict, kb, book_id: str, msg: str = "") -> str:
    ...
    try:
        ...
        entity_prompt = f"请从以下文本中提取所有实体名（人名/地名/物品名/组织名/功法名）:\n\n{text[:3000]}\n\n输出JSON数组:"
        raw = await loop.run_in_executor(
            _ai_executor, llm_chat, entity_prompt, entity_system, 0.1, "extraction"
        )
        import re
    ...
    try:
        ...
        mentioned_scores = [s for s in all_scores if s.entity_name in (text_entities if 'text_entities' in dir() else set())]
