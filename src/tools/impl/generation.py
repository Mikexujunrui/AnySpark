...
async def _generate_outline(loop, args: dict, book_id: str, queue=None) -> str:
...
async def _generate_worldbuilding(loop, args: dict, kb, book_id: str, queue=None) -> str:
    if queue:
...
async def _generate_location_map(loop, args: dict, kb, book_id: str, queue=None) -> str:
    if queue:
...
async def _generate_detailed_outline(loop, args: dict, book_id: str, queue=None) -> str:
    chapters_range = args.get("chapters", "all").strip()
    all_chapters = json_store.load_chapters(book_id)
    if not all_chapters:
        return "当前书籍没有章节。"
...
async def _generate_timeline(loop, args: dict, book_id: str, queue=None) -> str:
    outline = json_store.get_outline(book_id)
    chapters = json_store.load_chapters(book_id)
