...
async def _generate_outline(loop, args: dict, kb, book_id: str, msg: str = "", queue=None) -> str:
    ...
    while len(existing_extras) < total_extra_count:
        existing_extras.append(
            {"chapter_id": "", "title": "", "synopsis": "", "key_events": [], "characters": [], "notes": ""}
        )
    ...
    for idx in regular_indices:
        ...
        try:
            response = await asyncio.wait_for(
                loop.run_in_executor(_ai_executor, llm_chat, prompt, OUTLINE_SYSTEM, 0.1, "extraction"), timeout=120
            )
    ...
    try:
        ...
        extra_summaries = "\n".join(
            ...
            for i, e in enumerate(existing_extras)
            if e.get("synopsis")
        )
        if extra_summaries:
...
async def _generate_worldbuilding(loop, args: dict, kb, book_id: str, msg: str = "", queue=None) -> str:
    ...
    try:
        response = await asyncio.wait_for(
            loop.run_in_executor(_ai_executor, llm_chat, prompt, WORLDBUILDING_SYSTEM, 0.15, "extraction"), timeout=180
        )
    except Exception as e:
...
def _add_worldbuilding_entry_tool(args: dict, book_id: str) -> str:
    ...
    entry = {
        ...
        "tags": args.get("tags", []),
        "chapter_refs": args.get("chapter_refs", []),
    }
                ...
                if llm_connections:
                    ...
                    for c in llm_connections:
                        ...
                        if from_id and to_id:
                            ...
                            type_map = {
                                "contains": RelationType.LOCATED_IN,
                                "near": RelationType.ADJACENT_TO,
                                "path": RelationType.ADJACENT_TO,
                                "portal": RelationType.ADJACENT_TO,
                            }
