...
class ChapterStoreMixin:
    ...
    def add_chapter(self, book_id: str, title: str, content: str, is_extra: bool = False, status: str = "draft") -> dict:
        ...
        bus.emit_sync(Event(type=EventType.CHAPTER_CREATED, data={"book_id": book_id, "chapter_id": ch_id, "title": title, "is_extra": is_extra}))
        try:
    ...
    def edit_chapter(self, book_id: str, chapter_id: str, content: str,
                     title: str = None, message: str = "编辑") -> dict:
        with book_lock(book_id):
            chapters = self.load_chapters(book_id)
            ch = self._find_chapter(chapters, chapter_id)
            if not ch:
            ...
            if title:
                ch["title"] = title
            self.save_chapters(book_id, chapters)
        bus.emit_sync(Event(type=EventType.CHAPTER_UPDATED, data={
            "book_id": book_id, "chapter_id": ch["id"],
            "version": vid, "message": message}))
        try:
    ...
    def patch_chapter(self, book_id: str, chapter_id: str, patches: list[dict],
                      message: str = "局部编辑") -> dict:
        """局部编辑章节。
        ...
        with book_lock(book_id):
                ...
                try:
                    if op == "replace":
                        ...
                        if result is None:
                            ...
                            logger.warning("patch_chapter[%d] replace 未找到原文 | chapter=%s | anchor=%s | seg_id=%s",
                                           i, chapter_id, anchor[:50], seg_id)
                        ...
                        if result is None:
                            anchor = confirm or find
                            logger.warning("patch_chapter[%d] insert_after 未找到锚点 | chapter=%s | anchor=%s | seg_id=%s",
                                           i, chapter_id, anchor[:50], seg_id)
                        ...
                        if result is None:
                            anchor = confirm or find
                            logger.warning("patch_chapter[%d] insert_before 未找到锚点 | chapter=%s | anchor=%s | seg_id=%s",
                                           i, chapter_id, anchor[:50], seg_id)
                            failed_ops.append({"idx": i, "op": op, "reason": f"未找到锚点: {anchor[:30].strip()}…"})
                            continue
                        pos, matched = result
                        ...
                        if result is None:
                            anchor = confirm or find
                            logger.warning("patch_chapter[%d] delete 未找到文本 | chapter=%s | anchor=%s | seg_id=%s",
                                           i, chapter_id, anchor[:50], seg_id)
                            failed_ops.append({"idx": i, "op": op, "reason": f"未找到要删除的文本: {anchor[:30].strip()}…"})
                            continue
                        pos, matched = result
            ...
            if failed_ops:
                logger.warning("patch_chapter 完成 | chapter=%s | success=%d/%d | failed=%d",
                               chapter_id, patched_count, len(patches), len(failed_ops))
    ...
    def reorder_chapters(self, book_id: str, order: list[str]) -> dict:
        ...
        with book_lock(book_id):
            ...
            logger.info("reorder_chapters 完成 | book=%s | count=%d | old=%s | new=%s",
                        book_id, len(reordered), old_order, new_order)
    ...
    def revert_chapter(self, book_id: str, chapter_id: str, version_id: str) -> dict:
        ...
        try:
            ...
            fts_engine.index_chapter(book_id, {
                "id": ch["id"], "title": ch.get("title", ""), "content": current_content
            })
        except (OSError, RuntimeError) as e:
    ...
    def delete_chapter(self, book_id: str, chapter_id: str) -> int:
        ...
        if deleted:
            bus.emit_sync(Event(type=EventType.CHAPTER_DELETED, data={"book_id": book_id, "chapter_id": resolved_id, "count": deleted}))
        try:
