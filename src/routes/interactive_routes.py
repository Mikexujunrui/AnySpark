"""[DEPRECATED] Interactive Story API Routes — branch-based interactive storytelling.

.. deprecated:: 2.0
    This router is no longer registered in ``routes/__init__.py``.
    New code should use ``simulation_routes.py`` with the
    ``/books/{book_id}/simulation/`` prefix.

    This file is kept for reference but is NOT loaded at runtime.
"""
