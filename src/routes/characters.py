...
PHASE_FIELD_GROUPS = [
    ...
    ("内在", ["motivation", "驱动力", "fears", "likes", "dislikes", "goals", "secrets", "traumas", "角色主题", "成长弧光起点", "growth_note", "成长说明"]),
...
def _group_phase_data(data: dict) -> list[dict]:
    ...
    rest = [{"key": k, "label": _field_label(k), "value": v}
            for k, v in data.items() if k not in seen_keys and v]
    if rest:
...
def _field_label(key: str) -> str:
    labels = {
        ...
        "motivation": "驱动力", "relationships": "关系", "growth_note": "成长说明",
        "identity": "身份", "role": "角色定位", "origin": "出身",
        "temperament": "气质", "inner_conflict": "核心冲突",
        "fears": "恐惧", "goals": "目标", "secrets": "秘密",
        "traumas": "创伤", "key_experiences": "关键经历",
        "hair": "头发", "eyes": "眼睛", "height": "身高",
        "build": "体型", "clothing": "穿着", "distinctive_marks": "特征标记",
        "powers": "能力", "skills": "技能", "friends": "朋友",
        "affiliation": "归属", "likes": "喜好", "dislikes": "厌恶",
    }
...
def get_character_gallery(book_id: str):
    ...
    for c in chars:
        ...
        char_snapshots = [{
            "id": s.id, "timePoint": s.time_point, "timeOrder": s.time_order,
            "label": s.label, "data": s.data, "description": s.description,
            "phase": s.phase or "",
            "phaseKey": s.phase_key or "",
            "isCurrent": bool(s.is_current),
            "card": _group_phase_data(s.data),  # structured card sections for UI
        } for s in snapshots if s.character_entity_id == c.id]
    ...
    return {
        ...
        "timelineEvents": [{
            "id": t.id, "timePoint": t.time_point, "label": t.label,
            "timeOrder": t.time_order, "description": t.description,
...
def add_snapshot(book_id: str, entity_id: str, snap: SnapshotCreate):
    ...
    s = CharacterSnapshot(
        id=sid, character_entity_id=entity_id,
        time_point=snap.time_point, time_order=snap.time_order,
        label=snap.label, data=snap.data, description=snap.description,
        phase=snap.phase, phase_key=snap.phase_key,
...
def list_timeline_events(book_id: str):
    ...
    return [{"id": e.id, "timePoint": e.time_point, "label": e.label,
             "timeOrder": e.time_order, "description": e.description,
             "chapterRef": e.chapter_ref} for e in events]
...
def add_timeline_event(book_id: str, event: TimelineEventCreate):
    ...
    e = TimelineEvent(
        id=eid, time_point=event.time_point, label=event.label,
        time_order=event.time_order, description=event.description,
