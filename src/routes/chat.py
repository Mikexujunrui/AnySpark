...
# Shared logger with server.py's connection_diagnostics_middleware for correlation.
...
def _save_autopilot_sessions(sessions: dict[str, str]):
    try:
        _autopilot_sessions_file.write_text(json.dumps(sessions, ensure_ascii=False), encoding="utf-8")
    except OSError:
        pass
...
async def chat_with_agent(req: MessageRequest):
    ...
    elif intent == "generate_location_map":
        return EventSourceResponse(_tool_shortcut("generate_location_map", {}, req, msg))
...
- generate_location_map: 用户想生成地点地图（如"生成地图"、"创建地图"、"地点图"）
...
async def _tool_shortcut(tool_name: str, extra_args: dict, req, original_msg: str):
    ...
    while True:
        ...
        if isinstance(item, dict) and item.get("_progress"):
            yield {"event": "progress", "data": json.dumps({"stage": item["_progress"]}, ensure_ascii=False)}
        else:
            content = str(item) if item else "完成"
            _persist_turn(req.book_id, req.session_id, original_msg, content[:500], mode=req.mode)
            yield {"event": "done", "data": json.dumps({"message": content}, ensure_ascii=False)}
            return
...
async def _agent_loop_sse(msg: str, req: MessageRequest, handle=None):
    ...
    conn_logger.info(
        "[SSE-START] _agent_loop_sse | session=%s | mode=%s | msg_len=%d",
        sid,
        req.mode,
        len(msg),
    )
    ...
    try:
        # ── Bridge async generator to queue for cancellation support ──
        # When the SSE client disconnects, the generator is closed. Without
        # a task, run_agent_loop continues executing in the background.
        # Using a queue + task allows us to cancel the agent loop on disconnect.
        ...
        loop_task = asyncio.create_task(_drain_loop())
        ...
        while True:
            try:
                ...
                stalled = time.monotonic() - last_event_time
                if heartbeat_timeouts >= 3:
                    conn_logger.warning(
                        "[SSE-STALL] _agent_loop_sse | session=%s | %d consecutive timeouts | stall=%.1fs",
                        sid,
                        heartbeat_timeouts,
                        stalled,
                    )
                ...
                yield {
                    "event": "progress",
                    "data": json.dumps(
                        {"stage": "处理中...", "detail": "等待 Agent 响应 (轮次中...)"}, ensure_ascii=False
                    ),
                }
                continue

            heartbeat_timeouts = 0  # reset on successful receive
            last_event_time = time.monotonic()
        ...
        conn_logger.warning(
            "[SSE-DISCONNECT] _agent_loop_sse | session=%s | generator cancelled/closed",
            sid,
        )
        # Client disconnected or cancel was requested — stop the agent loop
        if loop_task and not loop_task.done():
            loop_task.cancel()
        if handle:
            handle.cancel()
        yield {"event": "done", "data": json.dumps({"message": "操作已中止", "cancelled": True}, ensure_ascii=False)}
        return
    finally:
        if loop_task and not loop_task.done():
            loop_task.cancel()
        # ── Critical: release session so subsequent messages aren't stuck
        #    in the "queued" state. Without this, the session stays RUNNING
        #    forever after the first SSE stream ends (normal or error exit).
        if sid and handle is not None:
            try:
                await run_state.release(sid, handle)
            except Exception as exc:
                logger.warning("Failed to release session %s: %s", sid, exc)
        elapsed = time.monotonic() - last_event_time
        conn_logger.info(
            "[SSE-END] _agent_loop_sse | session=%s | did_yield_done=%s | final_text_len=%d | idle_after_last_event=%.1fs",
            sid,
            did_yield_done,
            len(final_text),
            elapsed,
    ...
    if not did_yield_done:
        fallback_msg = (
            final_text or "Agent 循环异常终止，但未收到最终的完成信号。请检查后端日志获取详细信息，或尝试重新发送消息。"
        )
...
def _map_loop_event_to_sse(event: LoopEvent) -> dict | None:
    ...
    if t == "start":
        ...
        return {
            "event": "progress",
            "data": json.dumps({"stage": f"思考中... (轮次 {d.get('round', '?')})"}, ensure_ascii=False),
        }

    elif t == "compaction":
        return {"event": "progress", "data": json.dumps({"stage": d.get("stage", "压缩中")}, ensure_ascii=False)}

    elif t == "retry":
        return {
            "event": "progress",
            "data": json.dumps(
                {"stage": f"重试中 ({d.get('attempt')}/{3})", "detail": d.get("error", "")[:80]}, ensure_ascii=False
            ),
        }
        ...
        return {
            "event": "progress",
            "data": json.dumps(
                {"stage": f"调用: {d.get('tool')}", "detail": d.get("args", "")[:80]}, ensure_ascii=False
            ),
        }

    elif t == "progress":
        # Tool-level progress (e.g. extract_all_chapters chapter progress)
        return {
            "event": "progress",
            "data": json.dumps({"stage": d.get("stage", ""), "detail": d.get("detail", "")}, ensure_ascii=False),
        }
        ...
        return {
            "event": "progress",
            "data": json.dumps(
                {"stage": f"{d.get('tool')} 完成", "detail": d.get("result_preview", "")[:100]}, ensure_ascii=False
            ),
        }
        ...
        return {
            "event": "progress",
            "data": json.dumps(
                {"stage": f"工具错误: {d.get('tool')}", "detail": d.get("error", "")[:100]}, ensure_ascii=False
            ),
        }
        ...
        return {
            "event": "question",
            "data": json.dumps({"id": d.get("id", ""), "questions": d.get("questions", [])}, ensure_ascii=False),
        }
        ...
        return {"event": "progress", "data": json.dumps({"stage": f"权限拒绝: {d.get('tool')}"}, ensure_ascii=False)}

    elif t == "permission-granted":
        return {"event": "progress", "data": json.dumps({"stage": f"权限已确认: {d.get('tool')}"}, ensure_ascii=False)}
        ...
        return {"event": "done", "data": json.dumps({"message": d.get("message", "处理出错")}, ensure_ascii=False)}

    elif t == "cancelled":
        return {"event": "done", "data": json.dumps({"message": "操作已取消"}, ensure_ascii=False)}
...
def _classify_long_content(msg: str) -> str:
    ...
    hints = {
        ...
        "story_fragment": "用户提交了故事片段，可 manage_notes 保存。",
        "inspiration_note": "用户提交了灵感笔记，用 manage_notes 保存。",
    }
...
def _load_history_as_llm_messages(session_id: str, max_turns: int = 20) -> list[dict]:
    ...
    JSON store is canonical (auto-save persists the full history there).
    EventStore is a live replay buffer that may be truncated after auto-save,
    so it only serves as a fallback for sessions that haven't been saved yet.
    ...
    # Fall back to EventStore for sessions that haven't been saved yet
...
def _persist_turn(
    book_id: str, session_id: str, user_text: str, agent_text: str, mode: str = "", parts: list | None = None
):
    if not session_id:
        return
...
async def _extract_shortcut(text: str, kb, book_id: str, session_id: str = "", original_msg: str = ""):
    ...
    if not got_done:
        fallback_msg = "⚠️ 提取完成，但未收到结果数据。可能文本内容过短或格式不匹配，请提供更详细的角色设定文本。"
...
async def _handle_intervention(msg: str, req: MessageRequest, task_id: str, action: str):
    ...
    async def event_generator():
        if action == "pause":
            ...
            yield {
                "event": "done",
                "data": json.dumps({"message": "⏸ Autopilot 已暂停" if ok else "暂停失败"}, ensure_ascii=False),
            }
        elif action == "cancel":
            ...
            yield {
                "event": "done",
                "data": json.dumps({"message": "🚫 Autopilot 已取消" if ok else "取消失败"}, ensure_ascii=False),
            }
            ...
            yield {
                ...
                "data": json.dumps(
                    {
                        "message": f"📨 干预指令已注入，将在下一步生效：{msg[:50]}..."
                        if len(msg) > 50
                        else f"📨 干预指令已注入：{msg}"
                    },
                    ensure_ascii=False,
            ...
            yield {
                "event": "done",
                "data": json.dumps({"message": "已收到消息，将在 autopilot 下一步中处理"}, ensure_ascii=False),
            }

        # Persist the intervention turn
        if req.session_id:
            _persist_turn(req.book_id, req.session_id, msg, f"[autopilot 干预] {action}", mode=req.mode)
...
async def _autopilot_bridge_sse(task_id: str, book_id: str, session_id: str, request: Request = None):
    ...
    try:
        while True:
            # Check for client disconnection — prevents CLOSE_WAIT socket leaks
            if request and await request.is_disconnected():
                conn_logger.warning(
                    "[SSE-DISCONNECT] _autopilot_bridge_sse | task=%s | client disconnected",
                    task_id,
                )
                break
            ...
            try:
                ...
                if heartbeat_timeouts >= 3:
                    conn_logger.warning(
                        "[SSE-STALL] _autopilot_bridge_sse | task=%s | %d consecutive timeouts",
                        task_id,
                        heartbeat_timeouts,
                    )
            ...
            if event.type in (EventType.TASK_COMPLETED, EventType.TASK_FAILED):
                ...
                conn_logger.info(
                    "[SSE-END] _autopilot_bridge_sse | task=%s | event=%s",
                    task_id,
                    event.type,
                )
                return
        ...
        conn_logger.info(
            "[SSE-CLEANUP] _autopilot_bridge_sse | task=%s | listeners removed",
            task_id,
        )
