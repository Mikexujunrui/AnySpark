...
async def _agent_loop_sse(msg: str, req: MessageRequest, handle=None):
    ...
    try:
            ...
            try:
                ...
                if heartbeat_timeouts >= 3:
                    conn_logger.warning(
                        ...
                        sid, heartbeat_timeouts, stalled,
                    )
        ...
        if handle:
            handle.cancel()
        yield {"event": "done", "data": json.dumps(
            {"message": "操作已中止", "cancelled": True}, ensure_ascii=False)}
        return
    finally:
        if loop_task and not loop_task.done():
            loop_task.cancel()
        ...
        conn_logger.info(
            ...
            sid, did_yield_done, len(final_text), elapsed,
        )
