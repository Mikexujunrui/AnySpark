...
Endpoints:
  ...
  POST /api/books/{book_id}/analyses/deep-style?type=...&ref_book_id=...
  GET  /api/books/{book_id}/analyses/deep-style?type=...&ref_book_id=...
  POST /api/books/{book_id}/analyses/emotional-curve?ref_book_id=...
  GET  /api/books/{book_id}/analyses/emotional-curve?ref_book_id=...
...
def list_analyses(book_id: str):
    ...
    for ref_id in ref_ids:
        ...
        # Deep style: check any of the 4 dimensions
        deep_types = ["sentence_rhythm", "rhetoric_density", "prophecy_signature", "narrative_pov"]
        deep_count = sum(1 for dt in deep_types if load_analysis(dt, ref_id))
        if deep_count > 0:
            entry["deep_style"] = {"dimensions_analyzed": deep_count}
        # Emotional curve
        emotion = load_emotional_curve(ref_id)
        if emotion:
            entry["emotional_curve"] = {"chapter_count": emotion.get("chapter_count", 0)}
        if "structure" in entry or "style_fingerprint" in entry or "deep_style" in entry or "emotional_curve" in entry:
            results.append(entry)
    return {"analyses": results}


# ── Deep style analysis (sentence_rhythm, rhetoric_density, prophecy_signature, narrative_pov) ──

DEEP_ANALYSIS_TYPES = {
    "sentence_rhythm": ("句式韵律", "sentence_rhythm"),
    "rhetoric_density": ("修辞密度", "rhetoric_density"),
    "prophecy_signature": ("谶语特征", "prophecy_signature"),
    "narrative_pov": ("叙事视角", "narrative_pov"),
}


@router.post("/books/{book_id}/analyses/deep-style")
async def trigger_deep_style_analysis(
    book_id: str,
    analysis_type: str = "sentence_rhythm",
    ref_book_id: str | None = None,
):
    """Run a deep style analysis dimension. Results are cached."""
    if analysis_type not in DEEP_ANALYSIS_TYPES:
        raise HTTPException(400, f"不支持的分析类型: {analysis_type}。可选: {', '.join(DEEP_ANALYSIS_TYPES.keys())}")

    target_ref = _resolve_ref_book_id(book_id, ref_book_id)
    _, cache_key = DEEP_ANALYSIS_TYPES[analysis_type]
    ...
    try:
        ...
        result = analyzer(target_ref)
    except Exception as e:
        logger.exception("Deep style analysis (%s) failed for %s", analysis_type, target_ref)
        raise HTTPException(500, f"分析失败: {str(e)[:100]}")

    if not result.chapter_count:
        raise HTTPException(400, f"参考书 {target_ref} 没有章节内容，无法分析。")
    return result.to_dict()


@router.get("/books/{book_id}/analyses/deep-style")
def get_deep_style_analysis(
    book_id: str,
    analysis_type: str = "sentence_rhythm",
    ref_book_id: str | None = None,
):
    """Get cached deep style analysis. Returns 404 if not yet analyzed."""
    if analysis_type not in DEEP_ANALYSIS_TYPES:
        raise HTTPException(400, f"不支持的分析类型: {analysis_type}")
    target_ref = _resolve_ref_book_id(book_id, ref_book_id)
    _, cache_key = DEEP_ANALYSIS_TYPES[analysis_type]
    cached = load_analysis(cache_key, target_ref)
    if not cached:
        raise HTTPException(404, "尚未分析。请先 POST 触发分析。")
    return cached
...
@router.post("/books/{book_id}/analyses/emotional-curve")
async def trigger_emotional_curve(book_id: str, ref_book_id: str | None = None):
    """Run emotional curve analysis. Results are cached."""
    target_ref = _resolve_ref_book_id(book_id, ref_book_id)
    try:
        result = analyze_emotional_curve(target_ref)
    except Exception as e:
        logger.exception("Emotional curve analysis failed for %s", target_ref)
        raise HTTPException(500, f"分析失败: {str(e)[:100]}")
    if not result.chapter_count:
        raise HTTPException(400, f"参考书 {target_ref} 没有章节内容，无法分析。")
    return result.to_dict()
...
@router.get("/books/{book_id}/analyses/emotional-curve")
def get_emotional_curve(book_id: str, ref_book_id: str | None = None):
    """Get cached emotional curve. Returns 404 if not yet analyzed."""
    target_ref = _resolve_ref_book_id(book_id, ref_book_id)
    cached = load_emotional_curve(target_ref)
    if not cached:
        raise HTTPException(404, "尚未分析。请先 POST 触发分析。")
    return cached
