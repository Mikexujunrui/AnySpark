...
# Cache for system prompt token count (polled every ~500ms, rebuild is ~10ms)
_system_token_cache: dict = {"tokens": 0, "ts": 0.0}
...
def get_messages(book_id: str, session_id: str):
    ...
    # EventStore is a live replay buffer that may contain only the most recent
    # turn if the previous auto-save truncated it.  Always prefer the JSON store
    # when it has data; fall back to EventStore only for sessions that haven't
    # been saved yet.
    json_msgs = json_store.load_messages(session_id)
    if json_msgs:
        return json_msgs
...
def get_context_usage(book_id: str, session_id: str):
    # JSON store is canonical; EventStore is a fallback (same rationale as get_messages)
    messages = json_store.load_messages(session_id)
    if not messages and event_store.has_events(session_id):
    ...
    # Build the real system prompt and count its tokens. Cached for 60s
    # since it's rebuilt every ~500ms by the frontend poll.
    now = time.time()
    if now - _system_token_cache["ts"] > 60:
        from core.system_prompt import build_system_prompt

        system_prompt = build_system_prompt(agent_type="write")
        _system_token_cache["tokens"] = count_tokens(system_prompt)
        _system_token_cache["ts"] = now
    token_count = history_tokens + _system_token_cache["tokens"]
