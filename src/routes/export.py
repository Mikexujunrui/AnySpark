...
def export_book(book_id: str, format: str = "txt", metadata: bool = False):
    ...
    if format == "docx":
        ...
        return _make_download_response(
            data, f"{title}.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        )
    elif format == "spark":
        try:
            ...
            return FileResponse(
                ...
                headers={
                    "Content-Disposition": f"attachment; filename*=UTF-8''{quote(f'{title}.spark')}",
                },
            )
        ...
        try:
            ...
            raise HTTPException(500, str(e))
        return _make_download_response(data, f"{title}.epub", "application/epub+zip")
    else:
