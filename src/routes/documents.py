...
def validate_upload(filename: str, file_size: int) -> None:
    ...
    if file_size > MAX_FILE_SIZE:
        raise HTTPException(400, f"文件过大: {file_size / 1024 / 1024:.1f}MB > 200MB")


@router.post("/books/{book_id}/upload")
async def upload_document(book_id: str, file: UploadFile = File(...), session_id: str = Form("")):
    ...
    json_store.add_doc(sid, doc_id, safe_filename, len(full_text), str(save_path))

    return {
        "message": f"文件已就绪: {safe_filename} ({len(full_text)} 字, id: {doc_id[:8]})",
        "docId": doc_id,
...
async def detect_chapters(book_id: str, doc_id: str, session_id: str = ""):
    ...
    if chapters_data:
        return DetectChaptersResponse(
            method="regex",
            pattern=r"^第[一二三四五六七八九十百千万\d]+(?:章|回)[^\S\n]*.*$",
...
def _regex_split(full_text: str) -> list[dict]:
    chapter_heading = re.compile(
        r"^.*?(第[一二三四五六七八九十百千万\d]+(?:章|回))\s*(.*?)$|^(Chapter\s+\d+)\s*(.*?)$", re.MULTILINE
    )
