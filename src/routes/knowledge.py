...
def list_entities(book_id: str, entity_type: str | None = None):
    ...
    return [{"id": e.id, "name": e.name, "type": e.type, "aliases": e.aliases, "data": e.data} for e in entities]
        ...
        "foreshadows": [
            {
                ...
                "scheduled_chapter": f.scheduled_chapter,
            }
...
{{"valid": true/false, "conflicts": ["冲突描述1", ...], "notes": ["备注1", ...]}}"""
    ...
    response = chat(
        v_prompt, system="你是一位严格的校对员。只输出 JSON，不输出其他内容。", temperature=0.1, task="extraction"
    )
...
@router.post("/books/{book_id}/import-spark")
def import_spark_archive(book_id: str):
    """Import a .spark knowledge base archive into this book.

    The archive file is expected at ``data/archives/{book_id}.spark``
    (uploaded by the frontend beforehand).
...
@router.post("/books/{book_id}/upload-spark")
async def upload_spark_archive(book_id: str, file: UploadFile):
    """Upload a .spark archive file for later import.

    Saves the file to ``data/archives/{book_id}.spark``.
    """
...
def get_forgotten_characters(book_id: str, current_time_order: int = 0, threshold: int = 5):
...
def get_ontology_endpoint(book_id: str):
    ...
    return {
        "entity_types": [{"name": k, "label": v} for k, v in entity_labels.items()],
