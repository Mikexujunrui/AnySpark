...
def generate_wf(book_id: str, data: WorkflowGenerateRequest):
    ...
    # Persist
    json_store.add_workflow(book_id, wf.name, [
        {"id": s.id, "type": s.type, "label": s.label, "config": s.config}
        for s in wf.steps
    ])

    return {"id": wf.id, "name": wf.name, "steps": [
        {"id": s.id, "type": s.type, "label": s.label, "config": s.config} for s in wf.steps
    ]}
...
async def execute_wf(book_id: str, wf_id: str, data: WorkflowExecuteRequest = None):
    async def event_generator():
        ...
        if not wf:
            ...
            try:
                ...
                wf = wf_engine.build(wf_id, {
                    "name": wf_data.get("name", "工作流"),
        ...
        for i, step in enumerate(wf.steps):
            wf.current_step = i
            step.status = "running"
            yield {"event": "step_start", "data": json.dumps({"step": step.label, "index": i, "type": step.type}, ensure_ascii=False)}
            try:
                ...
                if handler:
                    ...
                    yield {"event": "step_done", "data": json.dumps({"step": step.label, "result": result}, ensure_ascii=False)}
                else:
                    step.status = "failed"
                    results.append({"step": step.label, "error": f"no handler for {step.type}", "id": step.id})
                    yield {"event": "step_error", "data": json.dumps({"step": step.label, "error": f"no handler: {step.type}"}, ensure_ascii=False)}
            except Exception as e:
                step.status = "failed"
                results.append({"step": step.label, "error": str(e), "id": step.id})
                yield {"event": "step_error", "data": json.dumps({"step": step.label, "error": str(e)[:200]}, ensure_ascii=False)}
