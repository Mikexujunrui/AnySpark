        ...
        try:
            while True:
                try:
                    ...
                    yield {
                        "event": "heartbeat",
                        "data": json.dumps({"task_id": task_id, "time": datetime.now().isoformat()}, ensure_ascii=False),
                    }
...
async def autopilot_chat_bridge(book_id: str, task_id: str):
    ...
    return EventSourceResponse(
        _autopilot_bridge_sse(task_id, book_id, task.session_id)
    )
